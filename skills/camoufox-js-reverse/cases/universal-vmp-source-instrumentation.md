...
- **JS 特征**：
  ...
  - `search_code(keyword='switch', script_url=<该文件>, context_chars=500)` 返回含 switch/case 的候选
...
- **反调试特征**：
  - debugger 陷阱（可 `inject_hook_preset(preset='debugger_bypass')` 绕过）
...
Step 8 — 读日志
  ...
  cookies(action='get')
  get_console_logs(keyword='COOKIE-HOOK')
...
- **坑 1：instrumentation(action='install') 必须在 navigate/reload 之前调用**——route 注册晚了就抓不到 VMP 脚本加载
...
- **坑 5：同一个 VMP 被多次加载（HMR / 页面跳转）** → 收紧 `url_pattern`，只改写必要脚本，避免重复开销
...
| obfuscator.io（开源） | case 数可能只有 20-30 | `search_code(keyword='switch', script_url=<VMP_URL>, context_chars=500)` |
