...
- **Cookie 归因分析**: `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` 融合 HTTP Set-Cookie 与 JS document.cookie 日志，一次性解答"这个 Cookie 到底是谁写的"（v2.5.0 新增）
...
将本目录放置到 AI 工具的 skills 目录下，AI Agent 会自动读取 `SKILL.md` 获取 JS 逆向分析能力。当你在对话中涉及接口签名分析、反爬对抗、动态 Cookie、混淆 JS、WASM 或浏览器环境调试等场景时，Skill 会自动激活。
...
本 Skill 围绕 `camoufox-reverse` MCP 服务器组织能力（Camoufox 反检测浏览器，统一 API）：
...
- Cookie 归因 `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` — 区分 Set-Cookie 与 document.cookie
...
### 调试工具
- **camoufox-reverse MCP**: 反检测浏览器逆向分析（含源码级插桩 + Cookie 归因分析）
