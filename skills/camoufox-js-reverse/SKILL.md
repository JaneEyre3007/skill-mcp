...
compatibility: "需要 camoufox-reverse MCP + Camoufox 作为指纹检测基准；CloakBrowser/js-reverse/chrome-devtools 只能作为辅助 CDP/SourceMap/Profiler/WebSocket/Hook 证据来源，不能替代 Camoufox 指纹基准"
argument-hint: "<目标URL> [需要分析的加密参数名, 如 sign, m, token]"
---

# 零级闸门（任何工具调用前先判断）

先只根据用户输入和当前会话判断任务是否真的属于本 skill。命中以下任一场景时直接转交，不进入后面的 Checklist：

0. **微信小程序 / PC 微信小程序 / WMPF / WeChatAppEx / AppService / WebView / 127.0.0.1:62000 / WMPFDebugger** → `wechat-miniapp-reverse`
1. 只要可粘贴的 Console / Snippets hook 脚本，或用户已经知道观察目标只缺脚本 → `browser-hook-snippets`
2. 已有 JS 文件或入口，目标是在 Node.js / VM / jsdom 中补环境跑通 → `env-patch`
3. 明确要紧凑 `Python + iv8 + requests` 脚本、iv8 翻页请求、iv8 执行 challenge/cookie/suffix → `iv8-web-reverse`
4. 明确是瑞数/Ruishu/Rivers 且已有 JS 入口，目标是在 Node runner、VM、jsdom 或最小 proxy 环境中跑通 → `env-patch`；明确要 iv8 执行 challenge/cookie/suffix → `iv8-web-reverse`；明确要完整采集器 → `web-protocol-recovery`
5. 目标是整文件 AST 解混淆、控制流还原、字符串数组展开或 Decode_action pass → `ast-deobfuscate`
6. 目标是端到端协议恢复、响应解码、WebSocket/GraphQL/protobuf、浏览器链路转 Python 采集器 → `web-protocol-recovery`
7. 上游 skill 或用户已给出真实请求、headers/cookie、调用栈、签名公式、固定输入输出或响应样本，且当前目标是“实现代码 / 写脚本 / 协议回放” → `web-protocol-recovery` 证据复用快路径

只有以上都不命中，且任务仍是“定位 Web API 加密/签名入口、采集环境读取证据、还原 JSVMP/签名算法”时，才继续执行本 skill 的硬约束 Checklist。

## 用户确认检查点

本 skill 默认可以连续完成侦察、定位和离线验证；但遇到下面情况必须先暂停并让用户确认，不要把“入口定位”滑坡成有副作用的业务操作：

1. 需要写入、导入或复用用户账号 Cookie、Authorization、session、localStorage 或其它登录态材料。
2. 需要触发真实提交、下单、支付、发送验证码、修改资料、批量翻页、高频请求或任何可能改变服务端状态的操作。
3. 需要采集、保存或展示完整 Cookie、token、Authorization、请求体、响应体、canvas/dataURL、设备指纹等敏感原文；默认只保存必要字段、长度、摘要或脱敏片段。
4. 需要处理验证码、可信输入、滑块、点选、人机校验或账号风控页面；先确认用户授权的交互边界和请求频率。
5. 入口和公式已经定位，下一步目标变成“写 Python/Node 采集器、协议回放、规模化采集”时，先转交 `web-protocol-recovery` 证据复用快路径，不在本 skill 内继续扩大实现范围。

无需暂停的安全动作：只读源码搜索、Network/initiator 查看、console 日志读取、离线固定输入输出验证、以及对已脱敏样本的本地算法推演。

# ⚠️ 硬约束 Checklist（分析启动前必做，不可跳过）

> **本段是确认任务属于本 skill 且不满足零级转交后才执行的最高优先级。AI 在第一次调用任何 MCP 工具之前，必须先在对话中以下面的原样复述这三项，并逐项输出执行结果。跳过复述或跳过任何一项视为违规，本次分析交付视为不合格。**
>
...
1. **任务已通过零级闸门确认属于本 skill 后，未做 CHECK-1 到 CHECK-3 的完整复述**，直接调用 `launch_browser` 或其他 MCP 工具
...
**Hook**：hook_function(mode='intercept'|'trace') / inject_hook_preset / remove_hooks / get_console_logs
   ⚠️ Firefox/Xray 下 `hook_function("JSON.stringify", ...)` 不可用，必须用 `window.fetch` 等可解析路径
...
- `get_request_initiator` 对 fetch 请求在 Firefox 下返回 null（已知限制），优先用 XHR 获取调用栈
...
第二板斧：插桩解释器（追踪执行链路）
  ...
  步骤 4：分层 hook_function(function_path=fn, mode='trace', max_captures=N)
           粗→中→细，逐步缩小范围
  步骤 5：trace_property_access(mode='summary') 或 hook_jsvmp_interpreter(mode='transparent')
           监控签名容器 + compare_env 采集环境基准；proxy 模式仅在明确需要且可接受挂浏览器风险时使用
...
**定制版安装**：从 camoufox-reverse releases 下载对应平台 zip，替换 Camoufox 缓存目录。不装对其他工具无影响。
...
Cookie 归因路径：
  ...
  → inject_hook_preset(preset="cookie", persistent=true)
   → 触发场景 → cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK') 手动归因
```
...
```
# 1. 确认是 VMP（命中密集 switch/case 解释器片段）
...
# 3. 一键归因
cookies(action='get') + list_network_requests 查看 Set-Cookie
→ 返回：
...
# ⚠️ proxy 模式在非 JSVMP 页面可能直接挂死浏览器，仅在有明确 JSVMP 证据且已安装定制版浏览器时使用
