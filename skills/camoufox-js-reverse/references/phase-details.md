...
MCP 操作：
  ...
  hook_function(function_path="加密函数名", mode='intercept', position='before', hook_code="console.log('[HOOK]', arguments)")
  get_console_logs               → 获取捕获的参数、返回值和调用栈
```
...
```
MCP 操作：
  hook_function(function_path="加密函数路径", mode='intercept', position='before', hook_code="console.log('[HOOK]', arguments)")
  → 通过 JS Hook 设置伪断点
  ...
  get_console_logs   → 获取伪断点捕获的参数和返回值
  get_console_logs(keyword='TRACE')        → 获取函数追踪数据
