...
[camoufox-reverse] scripts(action='save', url="混淆脚本URL", save_path="./obfuscated.js") → 保存到本地进一步分析
```
...
**MCP 辅助**：
```
[camoufox-reverse] hook_function(function_path="状态机入口函数", mode='intercept', position='before', hook_code="console.log('[STATE]', arguments)") → 设置伪断点
[camoufox-reverse] hook_function(function_path="状态机函数", mode='trace', log_args=true, log_return=true) → 追踪状态转移
[camoufox-reverse] get_console_logs(keyword='TRACE') → 查看状态值变化
```
...
**MCP 辅助**：
```
[camoufox-reverse] evaluate_js(expression="/* eval/Function Hook */")
...
**MCP 辅助**：
```
[camoufox-reverse] hook_function(function_path="解释器函数", mode='trace', log_args=true, log_return=true)
[camoufox-reverse] get_console_logs(keyword='TRACE') → 观察每步操作的输入输出
[camoufox-reverse] hook_function(function_path="解释器核心函数", mode='intercept', position='before', hook_code="console.log('[VM]', arguments)") → 捕获关键调用
```
...
[camoufox-reverse] evaluate_js → 注入全局 Hook
```
...
```
1. scripts(action='save') → 保存混淆代码到本地
2. search_code → 搜索可能的入口函数
3. hook_function(function_path="入口函数", mode='intercept', position='before', hook_code="console.log(arguments)") → 在入口设伪断点
4. get_console_logs → 查看捕获的参数和返回值
