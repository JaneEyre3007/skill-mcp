...
本手册详细说明 `camoufox-reverse-mcp` MCP 服务器的使用方式，
提供常见逆向场景下的具体操作步骤。

## 工具速查表

### camoufox-reverse-mcp（v1.1.0 当前工具集）

| 类别 | 工具 | 核心用途 |
|------|------|---------|
| **浏览器** | `launch_browser` / `close_browser` | 启动/关闭反检测浏览器 |
| **导航** | `navigate` / `reload` | 页面导航 |
| **页面交互** | `click` / `type_text` / `wait_for` | 元素交互 |
| **截图** | `take_screenshot` / `take_snapshot` | 截图 / 无障碍树 |
| **页面信息** | `get_page_info` / `evaluate_js("document.documentElement.outerHTML")` | 页面状态和 HTML |
| **源码搜索** | `search_code` | 在所有 JS 中搜索（结构化结果，默认 max_results=50） |
| **精确搜索** | `search_code(..., script_url=...)` | 在指定脚本中搜索（前后 3 行上下文，适合大文件） |
| **源码获取** | `scripts(action='get', ...)` | 获取指定脚本的代码 |
| **源码保存** | `scripts(action='save', ...)` | 将脚本保存到本地文件 |
| **脚本列表** | `scripts(action='list')` | 列出页面加载的所有脚本 |
| **执行 JS** | `evaluate_js` | 在页面执行任意 JS |
| **加载前注入** | `inject_hook_preset` | 页面加载前注入脚本（支持 `persistent` 跨导航重注入） |
| **伪断点** | `hook_function(mode='intercept')` | 通过 Hook 设置伪断点（支持 `persistent` 持久化） |
| **断点数据** | `evaluate_js` | 获取伪断点捕获的数据 |
| **函数追踪** | `hook_function(..., mode='trace', ...)` / `get_console_logs(keyword='TRACE')` | 追踪函数调用（支持 `persistent` 跨导航持久化） |
...
| **冻结原型** | `hook_function(..., mode='intercept', ...)` | 冻结任意原型方法，防止页面 JS 覆盖 Hook |
| **属性追踪** | `trace_property_access` / `hook_jsvmp_interpreter(mode='transparent')` | 引擎级属性追踪优先；proxy 模式高风险 |
| **JSVMP 插桩** | `hook_jsvmp_interpreter` | 一键插桩 JSVMP（Function.prototype.apply + 30+ 敏感属性） |
| **JSVMP 日志** | `evaluate_js("window.__mcp_jsvmp_log")` / `get_console_logs(keyword='TRACE')` | 获取 JSVMP 探针日志 |
| **JSVMP 字符串** | `scripts(action='get')` + 本地 AST/文本分析 | 提取字符串数组，识别 API 名称，检测混淆模式 |
| **环境采集** | `compare_env` | 全面收集浏览器环境（navigator/screen/canvas/WebGL/Audio/timing） |
| **网络捕获** | `network_capture(action='start', ...)` / `network_capture(action='stop')` | 启停网络捕获（支持 `capture_body` 捕获响应体） |
| **网络请求** | `list_network_requests` | 列出捕获的请求 |
| **请求详情** | `get_network_request` | 获取请求完整信息（含响应体） |
| **请求来源** | `get_request_initiator` | 获取请求的 JS 调用栈（当前 Firefox/Camoufox 下不稳定，需降级） |
| **请求拦截** | `intercept_request(action='log'|'block'|'modify'|'mock'|'stop')` | 拦截/修改/Mock/停止请求 |
| **Cookie** | `cookies(action='get', ...)` / `cookies(action='set', ...)` / `cookies(action='delete')` | Cookie 管理 |
| **Storage** | `get_storage` | 读取 localStorage / sessionStorage |
| **状态管理** | `export_state` / `import_state` | 保存/恢复浏览器状态 |
| **控制台** | `get_console_logs` | 读取控制台输出 |
| **指纹** | `compare_env` | 检查浏览器指纹 |
| **反检测** | `compare_env` / `trace_property_access` | 采集和验证指纹读取行为 |
| **反调试** | `inject_hook_preset(preset='debugger_bypass')` | 绕过 debugger 陷阱 |
| **分发循环扫描** | `search_code(keyword='switch', script_url=..., context_chars=500)` | **[v0.4.0]** 定位字节码分发函数（while+switch，case 数过滤） |
| **源码级插桩** | `instrumentation(action='install', ...)` | **[v0.4.0]** HTTP 层改写 VMP，每个 obj[key]/fn(args) 插入 tap（通用 VMP 利器） |
| **插桩日志** | `instrumentation(action='log', ...)` | **[v0.4.0]** 拉取源码插桩日志，带 hot_keys/hot_methods/hot_functions 摘要 |
| **插桩状态** | `check_environment()` / `instrumentation(action='stop', ...)` | **[v0.4.0]** 查看/停止源码插桩 |
| **Cookie 归因** | `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` | **[v0.4.0]** 融合 HTTP Set-Cookie + JS document.cookie + cookie jar |
| **hook 重载** | `instrumentation(action='reload')` | **[v0.4.0]** 重载使 persistent hook 先于页面 JS 执行（+ 清日志） |
| **运行时探针日志** | `get_console_logs` | **[v0.4.0]** 获取 runtime_probe 预设的广谱事件日志 |
...
步骤 8：获取请求调用栈（可用则走，不稳定时降级）
  [camoufox-reverse] get_request_initiator(request_id=N)
  → Firefox/Camoufox 下可能返回 null；失败时用 search_code + network 请求详情 + hook 日志定位
...
步骤 10：读取相关源码
  [camoufox-reverse] scripts(action='get', url="包含加密逻辑的脚本URL")

步骤 11：设置伪断点验证
  [camoufox-reverse] hook_function(function_path="加密函数路径", mode='intercept', position='before', hook_code="console.log('[HOOK]', arguments)")
  触发请求 →
...
步骤 4：根据调用栈定位生成函数
  ...
  [camoufox-reverse] scripts(action='get', url="...")

步骤 5：分析生成逻辑
  [camoufox-reverse] hook_function(function_path="cookie生成函数", mode='intercept', position='before', hook_code="console.log('[COOKIE-FN]', arguments)")
  刷新页面 →
  [camoufox-reverse] evaluate_js → 查看入参和返回值
  [camoufox-reverse] hook_function(function_path="加密函数", mode='trace', log_args=true, log_return=true, log_stack=true)
  [camoufox-reverse] get_console_logs(keyword='TRACE') → 查看中间值
```
...
```
步骤 1：列出所有脚本
  [camoufox-reverse] scripts(action='list')
  → 找到可疑的混淆脚本（通常文件名无意义或体积很大）

步骤 2：保存混淆脚本到本地
  [camoufox-reverse] scripts(action='save', url="混淆脚本URL", save_path="./project/obfuscated.js")
...
步骤 5：定位关键函数
  ...
  → 找到加密相关代码的大致位置
  [camoufox-reverse] scripts(action='get') → 获取上下文
```
...
步骤 4：追踪输入输出
  [camoufox-reverse] hook_function(function_path="wasm导出函数调用路径", mode='intercept', position='before', hook_code="console.log('[WASM]', arguments)")
  触发请求 →
  [camoufox-reverse] evaluate_js → 查看入参和返回值
...
步骤 1：启动浏览器 + 开启响应体捕获
  ...
  [camoufox-reverse] network_capture(action='start', capture_body=True)
...
步骤 3：确认是 VMP
  [camoufox-reverse] search_code(keyword='switch', script_url="https://target.com/xxx/sdenv-xxx.js", context_chars=500)
  → 命中密集 switch/case 解释器片段时，基本确认是 VMP

步骤 4：装源码级插桩（核心）
  [camoufox-reverse] instrumentation(action='install',
...
步骤 6：重载让插桩先于 VMP 生效
  [camoufox-reverse] instrumentation(action='reload')

步骤 7：触发业务操作（翻页 / 登录 / 搜索）

步骤 8：读 hot_keys / hot_methods / hot_functions（30 秒读完 VMP 指纹）
  [camoufox-reverse] instrumentation(action='log',
    tag_filter="vmp1",
...
步骤 9：Cookie 归因
  [camoufox-reverse] cookies(action='get')
  [camoufox-reverse] get_console_logs(keyword='COOKIE-HOOK')
  → 对每个 cookie 返回 sources（http_set_cookie / js_document_cookie）
    + http_responses[].url + js_writes[].stack

步骤 10：完工清理
  [camoufox-reverse] instrumentation(action='stop', url_pattern="**/sdenv-*.js")
...
> 对于RS、Akamai 这类"JS 算 token、服务端 Set-Cookie"的混合模式，单纯 `hook_function(Document.prototype.cookie)` 是抓不到最终 cookie 写入的——因为它是 HTTP header 写的，不经过 JS。`cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` 的价值就是 30 秒内消除这个盲区。

## 高级技巧

### 使用 hook_function(mode='trace') 自动记录函数调用
...
→ `get_console_logs(keyword='TRACE')` 会自动合并页面数据和 Python 端持久化数据
...
### 使用 hook_function(mode='intercept') 防止 Hook 被覆盖

```
[camoufox-reverse] hook_function(function_path="XMLHttpRequest.prototype.send", mode='intercept', position='before', hook_code="console.log('[XHR send]', arguments)", non_overridable=True)
```
...
```
[camoufox-reverse] hook_jsvmp_interpreter(mode="transparent")  → 安全模式探针
[camoufox-reverse] scripts(action='get', url="<VMP脚本URL>")    → 获取源码后本地提取字符串数组
  触发操作...
[camoufox-reverse] evaluate_js("window.__mcp_jsvmp_log")       → 获取页面侧探针日志
...
[camoufox-reverse] hook_jsvmp_interpreter(mode='proxy', track_props=True, proxy_objects=["navigator"])
  触发操作...
[camoufox-reverse] evaluate_js("window.__mcp_jsvmp_log") 或 trace_property_access(mode="summary")
```
→ 精确发现 JSVMP 或签名逻辑读取了哪些环境属性

### 使用 search_code(..., script_url=...) 精确搜索大文件

```
[camoufox-reverse] search_code(keyword="sign|token|encrypt", script_url=3)
```
→ 在指定脚本中搜索，返回前后 3 行上下文，适合 200KB+ 的 JSVMP 文件

### 使用 inject_hook_preset 进行预分析
...
`)         → 当前页面观察；跨导航需改成具体 preset 或 hook_function persistent=True
```
...
```
# 1. 先确认是不是 VMP（命中密集 switch/case 解释器片段）
[camoufox-reverse] search_code(keyword='switch', script_url="https://target.com/sdenv-xxx.js", context_chars=500)
...
# 4. 触发操作后读日志
[camoufox-reverse] instrumentation(action='log', tag_filter="vmp1", type_filter="tap_get", limit=200)
  → summary.hot_keys 是 VMP 读取的环境属性 top 30（指纹学习金矿）

[camoufox-reverse] instrumentation(action='log', tag_filter="vmp1", type_filter="tap_method", limit=200)
  → summary.hot_methods 是 VMP 调用的方法 top 30（ObjectType.methodName 格式）

# 5. 查看插桩状态
[camoufox-reverse] check_environment()

# 6. 完工移除
[camoufox-reverse] instrumentation(action='stop', url_pattern="**/sdenv-*.js")
[camoufox-reverse] instrumentation(action='stop')    # 不传参数 = 全部停止
```
...
| `instrumentation(action='install', ...)` | 每一次 member access + 每一次函数调用，无论是否通过可 hook API | 不跨脚本（只改写指定 url_pattern 的那个文件） |
| **建议**：两者同时开，互相校验 | | |
...
await instrumentation(action='install', url_pattern="**/sdenv-*.js",
...
await instrumentation(action='install', url_pattern="**/vmp.js", mode="ast_page")
```
...
# 触发操作
[camoufox-reverse] get_console_logs(keyword="xhr_send")
[camoufox-reverse] get_console_logs(keyword="canvas_toDataURL")   # canvas 指纹检测
[camoufox-reverse] get_console_logs(keyword="nav_read")           # navigator 读取
[camoufox-reverse] get_console_logs(keyword="addEventListener")   # 鼠标/键盘检测
...
| 页面导航 | `navigate` `reload` | 导航控制 |
| 页面交互 | `click` `type_text` `wait_for` | UI 自动化 |
| 页面状态 | `get_page_info` `take_screenshot` `take_snapshot` `evaluate_js("document.documentElement.outerHTML")` | 页面信息 |
| 源码分析 | `scripts(action='list')` `scripts(action='get', ...)` `search_code` `search_code(..., script_url=...)` `scripts(action='save', ...)` | JS 逆向核心 |
| JS 执行 | `evaluate_js` `inject_hook_preset` | 代码执行（支持持久化） |
| 调试 | `hook_function(mode='intercept')` `evaluate_js` `get_console_logs` | 伪断点调试（支持持久化） |
| Hook | `hook_function(..., mode='trace', ...)` `get_console_logs(keyword='TRACE')` `hook_function` `inject_hook_preset` `remove_hooks` `hook_function(..., mode='intercept', ...)` | 函数 Hook（支持持久化 + 防覆盖） |
| 属性追踪 | `trace_property_access` `hook_jsvmp_interpreter(mode='transparent')` | 引擎级/透明模式属性访问监控 |
| JSVMP 专项 | `hook_jsvmp_interpreter` `evaluate_js("window.__mcp_jsvmp_log")` `compare_env` | JSVMP 虚拟机保护分析 |
| 网络 | `network_capture(action='start', ...)` `network_capture(action='stop')` `list_network_requests` `get_network_request` `get_request_initiator` `intercept_request(action='stop')` | 网络分析（支持响应体捕获） |
| 存储 | `cookies(action='get', ...)` `cookies(action='set', ...)` `cookies(action='delete')` `get_storage` `get_storage` `export_state` `import_state` | 状态管理 |
| 反检测 | `compare_env` `trace_property_access` `inject_hook_preset(preset='debugger_bypass')` | 指纹与反检测 |
| 源码级插桩 | `instrumentation(action='install', ...)` `instrumentation(action='log', ...)` `check_environment()` `instrumentation(action='stop', ...)` `search_code(keyword='switch', script_url=..., context_chars=500)` | **[v0.4.0]** 通用 JSVMP 逆向利器 |
| Cookie 归因 | `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` | **[v0.4.0]** HTTP Set-Cookie + JS 写入融合分析 |
| 导航增强 | `navigate(pre_inject_hooks)` `instrumentation(action='reload')` `get_console_logs` | **[v0.4.0]** 首屏挑战/hook 重注入/运行时探针 |
