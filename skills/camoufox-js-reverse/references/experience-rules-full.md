...
分析动态 Cookie 第一步永远是 `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')()`，它能区分三种模式：
- 纯 JS 写入
- 纯 HTTP Set-Cookie
- JS 算 token + 服务端带回来
...
MCP 操作：
  ...
  <!-- v3.1.0: migrated from instrumentation(action='reload') -->
...
`instrumentation(action='install', ...)` 的 `last_mode_used` 长期是 `"regex (fallback)"` 说明目标有太新语法。
