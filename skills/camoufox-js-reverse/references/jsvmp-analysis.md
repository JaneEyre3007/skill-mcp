...
MCP 操作（推荐顺序）：
  ...
  - search_code(keyword='switch', script_url=<大 JS 的 url>, context_chars=500)   ← 首选 
    → 命中密集 switch/case 解释器片段时，基本确认是 VMP 解释器
  - scripts(action='get', url=<大 JS 的 url>) → 获取源码后本地提取字符串数组
    → 看源码是否包含 while/switch 解释器、navigator/screen/encrypt/md5/hmac 等关键词
...
兜底手动方式（大文件 search_code 超时时）：
  ...
  - scripts(action='get', url=<大 JS 的 url>) → 读取源码后按上下文定位分发体 
```
...
```
                   ...
                   │  cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')          │
                   └────────┬─────────────────────────┘
                            │
              夹逼 I/O ──→ ┌─┴─┐ ──→ 推断签名公式
                            │VMP│
  ...
  │    (mode='transparent')           │   └────────┬────────────────────────┘ 
                            ...
                            ↓                      ↓
                   ┌── 第三板斧 日志分析 ──────────────────┐
                    │  get_console_logs(keyword='TRACE') + evaluate_js("window.__mcp_jsvmp_log") │
                   │    + get_console_logs            │
...
Step 8 — cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK') 归因最终 cookie 来源
```
...
MCP 操作：
  ...
  - scripts(action='get') → 获取源码后本地提取字符串数组，识别 API 名称，检测混淆模式
  - 触发目标操作（翻页、提交等）
  - evaluate_js("window.__mcp_jsvmp_log") → 获取页面侧探针日志：API 调用、属性读取、调用时序
...
MCP 操作：
  ...
  - hook_function(className="XMLHttpRequest", methodName="send", mode='intercept') 
...
MCP 操作：
  ...
  - 触发一次包含签名参数的请求
  - get_console_logs(keyword='TRACE') → 获取 500 条执行记录（自动合并页面数据 + Python 端持久化数据）
...
MCP 操作：
  - hook_function(function_path="子函数A", mode='trace', log_args=true, log_return=true, log_stack=true) 
  - hook_function(function_path="子函数B", mode='trace', log_args=true, log_return=true, log_stack=true) 
  - 再次触发请求
  - get_console_logs(keyword='TRACE') → 分析子函数的输入输出
```
...
```
MCP 操作：
  - trace_property_access(mode='summary')
    → 引擎级属性访问追踪，自动记录读写操作
...
```javascript
// 通过 evaluate_js(persistent=True) 注入
...
```
MCP 操作：
  - get_console_logs(keyword='TRACE') → 获取函数追踪数据（自动合并页面数据 + Python 端持久化数据）
  - evaluate_js("window.__mcp_jsvmp_log") → 获取 JSVMP 专项日志（含 API 调用统计 + 属性读取摘要）
  - get_console_logs → 获取 Hook 输出
  - get_console_logs → 获取伪断点捕获数据
  - trace_property_access(mode='summary') → 获取属性访问记录
```
...
- Hook 更底层的接口：
  ...
  · 在 evaluate_js 中保存原始引用，在 VM 重写之前

- 或 Hook 网络层：
  · network_capture(action='start') → 不依赖 JS 层 Hook，直接在协议层捕获 
...
- 增加 hook_function(mode='trace') 的 max_captures 到 1000+ 
- 用时间戳过滤法缩小日志范围
```
...
第 3 步：VMP 劫持了整个请求链路 + hot_keys 显示大量环境读取？
  ...
  │   · cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK') 确认 cookie 是否来自 HTTP Set-Cookie
...
**第一板斧 — Hook 出入口**：
- [ ] `search_code(keyword='switch', script_url=..., context_chars=500)` 确认是否存在密集 switch/case 解释器片段 
...
- [ ] `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` 归因最终 cookie（HTTP vs JS）
- [ ] `inject_hook_preset("crypto", persistent=True)` 捕获加密原语入口（btoa/atob/CryptoJS/MD5/SHA）
- [ ] Hook String.fromCharCode（JSVMP 高频信号）
- [ ] `hook_function(..., mode='intercept', ...)` 防止 VMP 覆盖 hook 

**第二板斧 — 插桩解释器**：
- [ ] `hook_jsvmp_interpreter(script_url=<VMP basename>)` 多路径探针（apply/call/bind + Reflect.*/Proxy）
- [ ] 分层 `hook_function(mode='trace')`（粗 → 中 → 细，用 `max_captures` 限制日志量） 
- [ ] `trace_property_access(mode='summary')` 或 `hook_jsvmp_interpreter(mode='transparent')` 监控签名容器 

**第三板斧 — 日志分析**：
- [ ] `evaluate_js("window.__mcp_jsvmp_log")` + `get_console_logs(keyword='TRACE')` + `get_console_logs(keyword='runtime_probe')`
- [ ] 反向追踪法，找到签名值首次出现的位置
- [ ] 多次请求对比，确认变化因子和固定因子
...
**环境/时序**：
- [ ] 若目标是首屏挑战页，用 `navigate(pre_inject_hooks=[...])` 预注入必要 hook
- [ ] `compare_env` 采集环境基准
