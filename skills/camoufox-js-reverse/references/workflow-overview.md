...
- 通过 `inject_hook_preset` / `evaluate_js` 注入脚本截取中间数据
- 控制请求频率并结合 `compare_env` / `trace_property_access` 验证指纹与环境读取行为
...
4. 用 `search_code` 搜索参数名定位 JS 源码
   ↓
5. 优先用 `get_request_initiator`，失败时用 `get_network_request` + `search_code` + `hook_function(...)` 追踪调用栈
   ↓
6. 在关键函数设置 `hook_function(mode='intercept')` 或 `hook_function(mode='trace')`
   ↓
7. 单步调试确认加密逻辑
   ↓
8. 还原算法并对比中间值
...
2. 导航前判断是否首屏挑战
   ...
   │   → navigate(url=..., pre_inject_hooks=["xhr","fetch","cookie"])
   └─ 否
        → navigate(url=...) + 之后装 hook + instrumentation(action='reload')
   ↓
3. search_code(keyword='switch', script_url=<VMP_URL>, context_chars=500) 确认是否存在密集 switch/case 解释器片段
   ↓
4. 同时装三路探针：
   ...
   └─ 第四板斧（源码级插桩，核心）：instrumentation(action='install', 
                                     url_pattern="**/<VMP 文件>",
                                     mode="ast", tag="vmp1")
   ↓
5. instrumentation(action='reload') 让探针先于 VMP 生效 + 清日志
   ↓
6. 触发业务操作，让 VMP 生成一次签名
   ↓
7. 第三板斧读日志，分 3 个维度看：
   ├─ instrumentation(action='log', tag_filter="vmp1") → hot_keys / hot_methods / hot_functions
   ├─ evaluate_js("window.__mcp_jsvmp_log") → 页面侧 JSVMP 探针日志
   ├─ cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK') → cookie 归因（HTTP vs JS）
...
10. 连续 5+ 次请求验证稳定性
   ↓
11. instrumentation(action='stop') + remove_hooks() 清理
```
