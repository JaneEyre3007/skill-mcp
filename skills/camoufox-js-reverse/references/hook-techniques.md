...
```
[camoufox-reverse] evaluate_js(script=HookScript)
...
1. **使用 `evaluate_js`**：确保 Hook 在目标代码之前生效
...
9. **装完 hook 想让它先于页面 JS 跑**：用 `instrumentation(action='reload')` 替代裸 `reload()`，同时会清掉各类 `__mcp_*_log`
...
- **不适用**：HTTP Set-Cookie 写入的场景（这种用 `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` 的 http_responses 分支）
...
# 3. 拉日志（自动按 type 聚合）
[camoufox-reverse] get_console_logs(keyword="runtime_probe")

# 按类型过滤
[camoufox-reverse] get_console_logs(keyword="canvas_toDataURL")
[camoufox-reverse] get_console_logs(keyword="xhr_send")
[camoufox-reverse] get_console_logs(keyword="nav_read")
```

### 典型诊断模式

- **反爬是否做 canvas 指纹检测？** → 看 `by_type.canvas_toDataURL` 是否 > 0
- **反爬是否读 navigator.webdriver？** → `get_console_logs(keyword="nav_read")` 里找 `prop: "webdriver"`
