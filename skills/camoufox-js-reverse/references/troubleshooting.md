...
技巧 4：MCP 辅助对比
  - hook_function(function_path="签名函数", mode='intercept', position='before', hook_code="console.log('[SIGN]', arguments)")
  - get_console_logs → 获取浏览器端的真实入参和返回值
  - 与脚本端逐项对比
```
...
| 获取真实签名值 | `hook_function(mode='intercept')` + `get_console_logs` | 在签名函数设伪断点，捕获真实入参和返回值 |
| 验证环境检测 | `compare_env` + `compare_env` | 确认哪些环境项参与校验 |
| 追踪调用链 | `get_request_initiator` | 从请求直接定位到签名函数 |
| 实时对比 | `evaluate_js` | 在浏览器中执行还原后的签名函数，与脚本输出对比 |
| **Cookie 归因** [v2.5.0] | `cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')` | 辨识 Cookie 是 HTTP Set-Cookie 还是 JS document.cookie 写的 |
| **源码级插桩** [v2.5.0] | `instrumentation(action='install')` + `instrumentation(action='log')` | 对 VMP 做 HTTP 层源码改写，hot_keys 暴露环境指纹集 |
| **首屏挑战** [v2.5.0] | `navigate(pre_inject_hooks=[...])` | RS/Akamai 412 挑战页 hook 预注入 |
| **hook 重载** [v2.5.0] | `instrumentation(action='reload')` | 重载使 persistent hook 先于页面 JS 执行 + 清日志 |
| **运行时探针** [v2.5.0] | `get_console_logs` | 快速摸底页面在读什么 / 调什么（低开销） |
