...
> **v2.5.0 新增文档**。本指南讲解 camoufox-reverse MCP v0.4.0+ 提供的源码级插桩能力（`instrumentation(action='install')` / `instrumentation(action='log')` / `search_code(keyword='switch', script_url=..., context_chars=500)`）的使用方法论，是 JSVMP 四板斧中"第四板斧"的完整说明。
>
...
**`instrumentation(action='install')`** 在 HTTP 层拦截目标脚本，在脚本抵达浏览器前把源码改写成等价的"带 tap 的版本"：
...
`instrumentation(action='install', mode=...)` 支持两种模式：
...
- **失败回退**：AST 解析失败时（例如脚本有语法错误或 Acorn 版本不支持的语法），`instrumentation(action='install')` 会自动 pass-through 源码不变，并在 console 记 warn
...
> - 零 JS 依赖，`instrumentation(action='install')` 的 AST 模式现在**应成为默认选择**
...
启用 `instrumentation(action='install', mode="ast")` 后，通过 `instrumentation(action='status')` 的 `active_patterns[i].last_mode_used` 字段判断实际走了哪条路径：
...
Actions:
  ...
  network_capture(action='start', capture_body=True)   # 抓响应体，后续 cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK') 需要
```
...
Actions:
  ...
  → 如果命中密集 while/switch/case 解释器片段，基本确认是 VMP
  → 记下上下文中的分发函数名和局部代码位置（后续定位方便）
```
...
Actions:
  ...
  hook_jsvmp_interpreter(script_url="<VMP basename>",        # 多路径运行时探针
                         ...
                         track_reflect=True,
                         track_props=True)
  inject_hook_preset(preset='debugger_bypass')                                      # 避免 VMP 的 debugger 陷阱
```
...
```
Actions:
  instrumentation(action='reload', clear_log=True, wait_until="networkidle")
  → 注意 final_status：
    · 200   → 正常
...
instrumentation(action='log', tag_filter="vmp1", key_filter="webdriver")

# 只看 VMP 调用 CryptoJS.MD5 的记录（method_name 过滤）
instrumentation(action='log', tag_filter="vmp1", key_filter="MD5")
```
...
- VMP 内部 `obj[key]` 调用 → `instrumentation(action='log')`
- VMP 最终落到 `XMLHttpRequest.open` / `canvas.toDataURL` / `navigator.userAgent` 的 getter 调用 → `get_console_logs`
...
get_console_logs(keyword="xhr_send")
get_console_logs(keyword="canvas_toDataURL")
```
...
2. VMP 从缓存加载——强制 `instrumentation(action='reload', clear_log=True)` 或清浏览器缓存
3. pattern 注册太晚——要在 `navigate` / `instrumentation(action='reload')` **之前**调用 `instrumentation(action='install')`
...
- 或先 `scripts(action='save')` 把脚本存本地，人工看看是不是有奇怪的语法，再考虑局部 patch
...
2. 改写没成功——先 `instrumentation(action='status')` 看 `files_rewritten` 是否 > 0
3. tag 搞混了——检查 `tag_filter` 是否写对

### Q5：日志爆炸到上限 20000 条

**对策**：
- `instrumentation(action='log', clear=True)` 读完清
...
| hot_methods 特征 | hot_keys 环境属性数 | cookie 来源（cookies(action='get') + get_console_logs(keyword='COOKIE-HOOK')） | 策略 |
|-----------------|---------------------|--------------------------------------|------|
