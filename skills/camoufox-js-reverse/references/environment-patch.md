...
2. [camoufox-reverse] evaluate_js(script="记录被访问的全局变量和属性")
   + reload()
   + get_console_logs
   → 确定运行时真正访问了哪些环境字段

3. [camoufox-reverse] hook_function(function_path="环境检测函数路径", mode='intercept', position='before', hook_code="console.log('[ENV]', arguments)")
   + get_console_logs
