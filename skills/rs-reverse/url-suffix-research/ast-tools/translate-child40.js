 ...
 */
const fs = require('fs');
const { debugPath, readEvalCode } = require('./tool-paths');
const r2mka = JSON.parse(fs.readFileSync(debugPath('r2mka_parsed.json'), 'utf-8'));
const stringTables = JSON.parse(fs.readFileSync(debugPath('string_tables.json'), 'utf-8'));
const g72 = stringTables.g72;

const code = readEvalCode();
...
// 从反汇编提取每个 section
const disasm = fs.readFileSync(debugPath('child59_child40_disasm.txt'), 'utf-8').split('\n');
...
fs.writeFileSync(debugPath('cookie_s_parser.js'), output.join('\n'));
