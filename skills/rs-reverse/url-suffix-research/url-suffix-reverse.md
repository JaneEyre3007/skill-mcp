...
- 纯算客户端 `cookie-t-pure-runtime/runtime/pure-cookie-client.js` 已完美工作
...
### 6.2 如果只需要能用
- **JsRpc 方案** (`browser-rpc/`) — 已验证通杀商标站点 + 药监局
- **纯算客户端** (`cookie-t-pure-runtime/runtime/pure-cookie-client.js`) — 已验证完美工作（不需要后缀的站点）
...
```
rs-reverse/
├── cookie-t-pure-runtime/runtime/       ← ★ Cookie T 纯算（已完成，可用）
│   ├── pure-cookie-client.js            ← 纯算客户端
│   ├── cookie-t-e2e-runner.js           ← 全流程脚本
│   ├── vm-coder.js                      ← VM 重写
│   └── basearr-builder.js               ← 数据构建
├── browser-rpc/                ← ★ JsRpc 通杀方案（已验证）
│   ├── rpc-server.js
│   ├── browser-inject.js
│   └── rpc-client.js
├── url-suffix-research/              ← 后缀逆向研究（本目录）
│   ├── url-suffix-reverse.md ← 本文件
│   └── ast-tools/        ← AST 分析工具和结果
└── README.md             ← 项目总览
```
