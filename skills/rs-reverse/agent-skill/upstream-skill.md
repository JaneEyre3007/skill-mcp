...
> Reference only. This file is preserved from the upstream agent skill and is not an opencode-registered `SKILL.md`.
...
| 浏览器做的 | 纯算替代 | 阶段 | 详情 |
|-----------|---------|------|------|
| mainjs → eval 代码 | Coder 重写 | 3 | [coder-rewrite.md](guides/coder-rewrite.md) |
| cd → 提取 keys | extractKeys | 2 | [key-extraction.md](guides/key-extraction.md) |
| VM 收集指纹 → basearr | 数据驱动适配 | 4 | [basearr-adaptation.md](guides/basearr-adaptation.md) |
| basearr → 加密 → Cookie T | generateCookie | 1 | [encryption-chain.md](guides/encryption-chain.md) |
...
**产出**: 14 个 AST 工具, ~20h, 从 296KB 混淆代码中完整逆向后缀核心函数。详见 [ast-methodology.md](guides/ast-methodology.md)
...
5. 使用脚本: [tools/collect-type2.js](tools/collect-type2.js)
...
使用 [tools/collect-session.js](tools/collect-session.js) 一次性采集:
...
| 文件 | 内容 |
|------|------|
| [guides/encryption-chain.md](guides/encryption-chain.md) | 阶段 1: Huffman/AES/CRC32/Base64 完整加密解密实现 |
| [guides/key-extraction.md](guides/key-extraction.md) | 阶段 2: cd 解码 + XOR 偏移推导 + 45 组 keys 提取 |
| [guides/coder-rewrite.md](guides/coder-rewrite.md) | 阶段 3: 外层 VM 重写 + 75+55 opcode + 9 步调试过程 |
| [guides/basearr-adaptation.md](guides/basearr-adaptation.md) | 阶段 4: TLV 结构 + 每个 type 实现 + 数据驱动案例 |
| [guides/ast-methodology.md](guides/ast-methodology.md) | AST 反编译 4 步流水线 + 14 个工具 + 方法对比 |
| [guides/vm-hook-cookbook.md](guides/vm-hook-cookbook.md) | 7 种 VM 注入技术 + console 侧信道导出 |
| [guides/suffix-analysis.md](guides/suffix-analysis.md) | URL 后缀 88B/120B 结构 + SHA-1 签名 + 现有方案 |

### 可执行脚本

| 文件 | 用途 |
|------|------|
| [tools/collect-session.js](tools/collect-session.js) | 配套数据一次性采集 (sdenv + VM 注入) |
| [tools/hybrid-verify.js](tools/hybrid-verify.js) | 混合验证: sdenv basearr + 纯算加密 → 200 |
| [tools/pure-run.js](tools/pure-run.js) | 纯算全流程模板 (零第三方依赖) |
| [tools/collect-type2.js](tools/collect-type2.js) | type=2 多 session 采集 + 映射推导 |
| [tools/sdenv-client.js](tools/sdenv-client.js) | sdenv 客户端 (Cookie + VM 内 XHR) |

### 参考实现

| 文件 | 说明 |
|------|------|
| [reference-impl/coder.js](reference-impl/coder.js) | 外层 VM 重写器 (362 行, 验证: eval 代码 100% 字节一致) |
| [reference-impl/basearr.js](reference-impl/basearr.js) | basearr 生成器 (304 行, 验证: HTTP 200) |
