...
| 序号 | 脚本 | 功能 | 输入 | 输出 | 耗时 |
|------|------|------|------|------|------|
| 1 | extract-opcodes.js | 从 _$_I 提取 114 个 opcode | eval_code.js | opcodes.json (409 条目) | 2h |
| 2 | verify-rt-map.js | 验证 440 个 rt[] 映射 | eval_code.js | rt_map.json | 1h |
| 3 | decompose-bs-children.js | 深入分析 rt[239] 后缀生成器 | eval_code.js + rt239_source.js | 调用链 + 关键函数 | 3h |
| 4 | trace-rt239.js | 追踪 rt[239] 的完整子函数 | eval_code.js | 函数源码集 | 2h |
| 5 | trace-session49.js | 追踪 Cookie S → 49B session | eval_code.js | 解密调用链 | 2h |
| 6 | find-xtea-huffman.js | 定位 XTEA/Huffman/SHA-1 | eval_code.js | 算法函数定位 | 1h |
| 7 | trace-49b-session.js | 追踪 49B session 数据流 | eval_code.js | 数据流图 | 2h |
| 8 | session-chain.js | Cookie S 完整解密链路 | eval_code.js | 递归调用链 | 2h |
| 9 | cookie-s-decrypt.js | Cookie S 解密路径 | eval_code.js | AES/XTEA 定位 | 1.5h |
| 10 | cookie-s-complete.js | Cookie S 完整处理 | eval_code.js | 端到端流程 | 2h |
| 11 | r2mka-disasm.js | r2mKa 字节码反汇编 | r2mka_parsed.json | 6328 行汇编 | 3h |
| 12 | bytecode-to-js.js | 字节码 → 伪 JS (栈模拟) | r2mka_parsed.json | 1653 行伪 JS | 4h |
| 13 | translate-child40.js | 翻译 Cookie S TLV 解析器 | r2mka_parsed.json | 可读 JS 函数 | 2h |
| 14 | suffix-structure.js | 后缀 88B/120B 结构分析 | rt239_source.js | URL 编码流程 | 3h |
