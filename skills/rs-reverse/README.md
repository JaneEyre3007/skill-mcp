...
> 没有猜, 全是数据对出来的 — 这就是 `agent-skill/upstream-skill.md` 中的「数据驱动方法论」。

### 涉及文件

| 文件 | 改了什么 | 是否必须 |
|------|---------|---------|
| `cookie-t-pure-runtime/runtime/basearr-builder.js` | `buildType2()` 中的 `indexToValue` 映射表 | **必须** — 这是 412 的唯一原因 |
| `cookie-t-pure-runtime/runtime/cookie-t-e2e-runner.js` | UA 字符串 + `platform` 参数 | 可选 — 与 sdenv 真实数据对齐 |
| `cookie-t-pure-runtime/runtime/pure-cookie-client.js` | UA 字符串 + `platform` 默认值 | 可选 — 与 sdenv 真实数据对齐 |
...
4. 更新 `basearr-builder.js` 中的对应值

完整方法论见 `agent-skill/upstream-skill.md` 中的「方法论」章节。参考采集脚本见 `agent-skill/tools/collect-session.js`。
...
```bash
cd cookie-t-pure-runtime/runtime
node pure-cookie-client.js
```

```javascript
const { RuishuPureClient } = require('./cookie-t-pure-runtime/runtime/pure-cookie-client.js');
...
```bash
cd browser-rpc && npm install

# 1. 启动服务端
node rpc-server.js

# 2. 浏览器打开目标站点 → F12 → Console → 粘贴 browser-inject.js

# 3. 调用
node rpc-client.js                          # 自动测试
node rpc-client.js get /path                    # GET
node rpc-client.js post /api "key=value"        # POST (自动带后缀)
```
...
rs-reverse/
│
├── cookie-t-pure-runtime/                         ★ 纯算实现 (核心成果)
│   ├── runtime/
│   │   ├── pure-cookie-client.js    纯算客户端 (233行, 一键使用)
│   │   ├── cookie-t-e2e-runner.js   Cookie T 全流程脚本
│   │   ├── vm-coder.js              外层 VM 重写 (362行)
│   │   ├── basearr-builder.js       明文数据构建 (304行)
│   │   └── pure-e2e-verify.js       端到端验证
│   ├── samples/
│   │   └── mainjs-sample.js         缓存的 mainjs
│   ├── tests/
│   │   └── api-test.js              API 接口测试
│   └── research-archive.md          完整逆向过程记录
│
├── url-suffix-research/                         ★ URL 后缀 AST 深度逆向
│   ├── ast-tools/                   14 个 AST 分析工具 + 1 个路径辅助
│   │   ├── trace-rt239.js       定位后缀核心函数 rt[239] (15KB)
│   │   ├── decompose-bs-children.js           拆解 56 个子函数
│   │   ├── extract-opcodes.js   提取 409 个 VM opcodes
│   │   ├── r2mka-disasm.js      自动反汇编 (6328行输出)
│   │   ├── bytecode-to-js.js    栈模拟翻译 (字节码→伪JS)
│   │   ├── suffix-structure.js  后缀 88B/120B 结构分析
│   │   ├── find-xtea-huffman.js 定位加密函数 (SHA-1/XTEA)
│   │   ├── verify-rt-map.js        440 个 rt[N] 完整映射
│   │   ├── session-chain.js     AES 解密链提取
│   │   ├── cookie-s-decrypt.js  Cookie S 解密路径
│   │   ├── cookie-s-complete.js 核心函数翻译
│   │   ├── translate-child40.js TLV 解析器翻译
│   │   ├── trace-session49.js   49B session 追踪
│   │   ├── trace-49b-session.js         纯 AST 完整 49B 路径
│   │   └── tool-paths.js                共享路径辅助，避免本机绝对路径
│   └── url-suffix-reverse.md            后缀逆向完整记录
│
├── browser-rpc/                           ★ JsRpc 通杀方案
│   ├── rpc-server.js                WebSocket RPC 服务端
│   ├── browser-inject.js            浏览器注入脚本
│   ├── rpc-client.js                Node.js 客户端
│   └── README.md                    使用说明
│
├── sdenv-sandbox/                   补环境方案 (魔改 jsdom)
│   ├── sdenv-client.js              sdenv 客户端
│   └── README.md                    技术总结
│
├── agent-skill/                      ★ 顶级逆向 Skill (可复制给任何 AI Agent)
│   ├── upstream-skill.md            上游主文档 (362行, 决策树+方法论+排错指南)
│   ├── guides/                  按需加载的详细参考 (7 篇)
│   │   ├── encryption-chain.md      阶段1: 加密链 (Huffman/AES/CRC32/Base64)
│   │   ├── key-extraction.md        阶段2: 密钥提取
│   │   ├── coder-rewrite.md         阶段3: 外层VM重写 + 调试6个坑
│   │   ├── basearr-adaptation.md    阶段4: basearr数据驱动适配
│   │   ├── ast-methodology.md       AST反编译4步流水线
│   │   ├── suffix-analysis.md       URL后缀结构+SHA-1签名
│   │   └── vm-hook-cookbook.md       7种VM注入技术
│   ├── tools/                       可直接执行的脚本 (5 个)
│   │   ├── collect-session.js       配套数据一次性采集
│   │   ├── hybrid-verify.js         混合验证 (证明加密链正确)
│   │   ├── pure-run.js              纯算全流程模板
│   │   ├── collect-type2.js         type=2多session采集
│   │   └── sdenv-client.js          sdenv客户端
│   └── reference-impl/              格式化的参考实现
│       ├── coder.js                 外层VM重写器 (508行, 验证100%字节一致)
│       └── basearr.js               basearr生成器 (282行, 验证HTTP 200)
│
├── upstream-rs-reverse/                  pysunday/rs-reverse 开源参考 (纯算借鉴来源)
...
### 1. 纯算 Cookie T (cookie-t-pure-runtime/runtime/)

**完全不依赖浏览器**, 纯 Node.js 实现整条加密链。

本方案借鉴了 [pysunday/rs-reverse](https://github.com/pysunday/rs-reverse) 开源项目 (已收录在 `upstream-rs-reverse/`), 该项目已在 9+ 个瑞数站点验证通过。我们在其基础上:
...
核心文件:
- `vm-coder.js` — 外层 VM 重写 (362行), 从 mainjs 生成 eval 代码, 逐字节与原始一致
- `basearr-builder.js` — 明文数据构建 (304行), 8 种 TLV type 全覆盖
- `pure-cookie-client.js` — 完整客户端 (233行), Cookie 自动更新
...
```
agent-skill/
├── upstream-skill.md (362行) ← 上游方法论主文档
...
│
├── guides/ (7篇)         ← 按需加载, 不污染 context window
│   加密链 / 密钥提取 / Coder重写 / basearr适配
│   AST方法论 / 后缀分析 / VM注入手册
│
├── tools/ (5个)              ← 可直接 node 执行
│   数据采集 / 混合验证 / 纯算全流程 / type=2采集 / sdenv客户端
│
└── reference-impl/ (2个)     ← 格式化的参考实现
    coder.js (508行) + basearr.js (282行)
```

**对比原来的单文件方案**: 主文档从 3300 行 / 53K tokens 压缩到 **362 行 / ~6K tokens** (缩减 89%), AI 首次加载成本降低一个数量级, 同时通过 guides/ 保留了所有深度内容。
...
**给 AI Agent**: 把 `agent-skill/` 目录放到项目中, Agent 读 `upstream-skill.md` 即可开始工作。

**给人类**: 读 `upstream-skill.md` 的决策树和排错指南, 按 Checklist 适配新站点。
...
完整的 22 步 hook 进化过程见 `cookie-t-pure-runtime/research-archive.md`。
