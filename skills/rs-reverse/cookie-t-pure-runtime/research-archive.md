...
### 生产脚本 (`cookie-t-pure-runtime/runtime/`)
| 文件 | 说明 |
|------|------|
| `cookie-t-e2e-runner.js` | 全动态入口 (自动下载 mainjs) |
| `pure-e2e-verify.js` | 本地版入口 (依赖 samples/mainjs-sample.js) |
| `vm-coder.js` | 外层 VM 重写器 |
| `basearr-builder.js` | basearr 生成器 |

### 原始逆向脚本 (`cookie-t-pure-runtime/`)
...
| collect-type2.js | type=2 多 session 采集 |
