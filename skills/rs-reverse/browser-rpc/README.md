...
```
browser-rpc/
├── rpc-server.js      ← 服务端 (需要 ws 库: npm install)
├── browser-inject.js  ← 浏览器注入脚本
├── rpc-client.js      ← Node.js 客户端 (自动检测站点)
└── README.md
```
...
1. 打开目标网站
2. F12 → Console
3. 粘贴 browser-inject.js 全部内容，回车
4. 看到 `[RPC] ★ 已连接` + `[RPC] ★★★ 就绪!`
...
# GET 请求
node rpc-client.js get /xxgk/ggtg/index.html

# POST 请求
node rpc-client.js post /api/data "page=1&rows=10"

# 获取 Cookie
node rpc-client.js cookie

# 获取后缀
node rpc-client.js suffix /path
```
