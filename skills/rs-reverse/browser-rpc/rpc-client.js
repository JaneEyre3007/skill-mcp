 ...
 * JsRpc 调用客户端
 *
 * 前置: node rpc-server.js + 浏览器注入 browser-inject.js
 *
 * 用法:
 *   node rpc-client.js                 — 自动检测站点并测试
 *   node rpc-client.js get /path           — GET 请求
 *   node rpc-client.js post /path "body"   — POST 请求
 *   node rpc-client.js cookie              — 获取 Cookie
 *   node rpc-client.js suffix /path        — 获取后缀
 */
...
async function testAuto() {
    ...
    try {
        ...
        console.log('\n请确保:');
        console.log('  1. rpc-server.js 正在运行');
        console.log('  2. 浏览器已注入 browser-inject.js');
    }
