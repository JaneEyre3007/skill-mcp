...
`Web Protocol Recovery` 是一套面向 Web 协议恢复与参数还原场景的逆向工程 Skill。
...
`Web Protocol Recovery` 解决的不是“怎么自动点按钮”，而是下面这类问题：
...
当前版本围绕 `chrome-devtools-mcp` 与 `js-reverse-mcp` 展开，`js-reverse-mcp` 默认使用普通 Chrome，需要时通过 `launch_browser` 运行时切换到 CloakBrowser。强调普通 Chrome 优先、离线还原、Python 优先交付。
...
为了保证最终交付可靠、可迁移、可维护，`Web Protocol Recovery` 明确不把以下做法视为合格答案：
...
`Web Protocol Recovery` 的最短主线可以概括为五步：

1. 先做 `Startup Gate`
2. 用 `chrome-devtools-mcp` 与普通 `js-reverse-mcp` 做轻量侦察，需要指纹证据时再通过 `launch_browser` 切换到 CloakBrowser
3. 找到真实请求与真实动态状态
4. 在本地离线重建这些动态状态
...
- `js-reverse-mcp` 负责 initiator、源码搜索、wrapper 追踪、首轮 mutation 假设
...
`Web Protocol Recovery` 不接受“这次跑通了”作为完成条件。
...
`Web Protocol Recovery` 的优势不在“工具多”，而在“把复杂目标拆成稳定问题”。
...
```bash
git clone <your-repo-url> ~/.agents/skills/web-protocol-recovery
```
...
建议在以下场景触发 `Web Protocol Recovery`：
...
一个通过 `Web Protocol Recovery` 完成的任务，理想上应包含以下产物：
...
`Web Protocol Recovery` 不是让浏览器替你干活。
