...
1. environment and tool sanity
   ...
   - confirm whether `chrome-devtools`, and `js-reverse-mcp` (normal Chrome, switchable to CloakBrowser via `launch_browser`) are usable
   - use `js-reverse-mcp` for ordinary work; escalate to CloakBrowser via `launch_browser({cloakBinaryPath: "..."})` only after fingerprint evidence
   - remember that browser switching is now runtime (no separate MCP server needed)
...
2. artifact directory discipline
   ...
   - save downloaded JS, WASM, fonts, HTML, network exports, state snapshots, screenshots, traces, and fixed-input vectors under that project's `js_reverse_cache/`
   - keep final collector code in the same dedicated protocol project directory under `collector/`, not mixed into the cache or workspace root
3. family triage
...
Do not switch to CloakBrowser merely because the target is a JS reverse task.

Escalate only after a normal-Chrome baseline (from `js-reverse-mcp` or `chrome-devtools-mcp`) or source evidence indicates fingerprint checks, automation detection, verifier behavior, or environment-dependent output. Invoke `launch_browser({cloakBinaryPath: "D:\\develop_software\\CloakBrowser\\chrome.exe"})` to switch; the browser closes and relaunches on the next tool call. Record the trigger before upgrading so later work can distinguish protocol requirements from tooling convenience.
