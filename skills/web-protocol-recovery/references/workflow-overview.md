...
- create or confirm the generated project root's `js_reverse_cache/` for dynamic reconnaissance artifacts
- capture the first browser baseline in normal Chrome unless it is unavailable
- treat CloakBrowser as an escalation tier for proven fingerprint, anti-bot, or environment-sensitive failures
...
- normal-Chrome baseline vs CloakBrowser escalation evidence
...
- final code and reconnaissance artifacts in one dedicated scaffolded collector project: code under `collector/`, curated notes under `analysis/`, raw dynamic artifacts under `js_reverse_cache/`
