...
使用本 skill 当用户的目标对象是 `SKILL.md`、skill 目录、skill description、trigger boundary、eval/test prompt、benchmark 或优化历史。

不要使用本 skill 当：

1. 用户只是要执行某个领域任务，例如逆向网站、写 hook、补环境、做小程序调试或生成业务代码；这类任务应交给对应领域 skill。
2. 用户在改 `opencode.json`、MCP server、agent/subagent、插件或权限规则，但没有修改 skill 定义；这类任务交给 opencode 配置相关流程。
3. 用户只问某个 skill 怎么用，且没有要求评分、优化、改写或测试；直接解释即可，不启动优化循环。
4. 目标 skills 目录没有 git 仓库或备份回滚基线，且用户没有选择回滚策略；只能做只读评估，不能编辑 `SKILL.md`。

先判定运行模式：

1. `evaluate-only`：用户说“评估/打分/看看质量/不要改”，只执行测试 prompt 设计和基线评分，不进入 Phase 2。
2. `history-only`：用户说“查看历史/结果怎么样”，只读 `auto-optimize-results.tsv` 或用户指定 results 文件。
3. `optimize`：用户明确要求优化、改进、自动优化、修 trigger 或提升质量，才进入可编辑循环。

模式不明确时先问一句，不要默认从只读问题升级为编辑文件。
...
```
1. 确认优化范围：
   - 全部skills → 优先扫描当前 OpenCode skills 根目录下的 */SKILL.md
   - 常见根目录：~/.config/opencode/skills/、~/.config/opencode/skill/、项目 .opencode/skills/
   - 兼容外部来源：~/.claude/skills/*/SKILL.md、~/.agents/skills/*/SKILL.md
   - 指定skills → 用户指定列表
...
OpenCode 执行约束：如果目标 skills 目录不是 git 仓库，先暂停并让用户选择：初始化 git、使用备份文件回滚，或只做基线评估。不要在没有回滚基线时编辑 `SKILL.md`。
...
OpenCode 子agent规则：优先用 `task` 工具启动独立 `general` 子agent 分组评分；提示词必须要求只读、读取目标 `SKILL.md` 和 `test-prompts.json`、返回结构化 JSON、标注 `eval_mode`。如果不能启动子agent或测试会触发真实网络/浏览器/外部副作用，则改用 `dry_run`，并在 `results.tsv` 记录原因。
...
文件位置：目标 skills 根目录的 `auto-optimize-results.tsv`。如果用户明确要求沿用 Claude 目录，再写入 `.claude/skills/auto-optimize-results.tsv`。
