# Optional advanced-env-engine Path

默认优先使用当前 skill 的 `scripts/vm-browser-gap-diagnose.js` 和 `env/` 模块树。只有在下面情况出现时，再启用 `scripts/advanced-env-engine.js` 这条高级手写补环境路径：

1. 需要 `Function.prototype.toString` / `Symbol.toStringTag` / 构造器外形这类 native 伪装。
2. 模块化 env 已能加载，但签名格式与浏览器仍不一致，需要定向监控少数对象。
3. 用户已有明确入口，且希望先在缓存区验证，再把补丁集中合并到项目根目录 `mod.js` / `main.js`。
4. webpack 模块已定位，需要配合 `scripts/webpack-module-runtime.js` 提取和加载模块。
...
```text
执行代码工作区/
├── mod.js             # 最终验证通过后沉淀的环境文件
├── main.js            # 最终验证通过后沉淀的 JS 入口
└── js_reverse_cache/
    ├── target/          # 原始 JS、入口片段、fixtures
    ├── env/
    │   ├── advanced-env-engine.js
    │   ├── verify-entry.mjs   # 缓存区验证入口
    │   └── ai-generated/
    └── docs/
        └── progress.md
```
...
`advanced-env-engine.js` 是 ESM `.js` 文件。复制到目标项目后，优先在 `js_reverse_cache/env/package.json` 写入 `{ "type": "module" }`，让缓存区验证文件、`advanced-env-engine.js` 和可选的 `webpack-module-runtime.js` 都按 ESM 运行；不要强迫根目录 `main.js` / `mod.js` 跟着切到 ESM，除非项目本身已经是 ESM。
...
1. `advanced-env-engine.js` 复制后不改；所有站点补丁先写在缓存区验证文件，验证通过后再合并到根目录 `mod.js`。
2. 环境必须在 `await import('./main.js')` 之前初始化。如果目标副本必须用 CommonJS 加载，改用 `createRequire(import.meta.url)`，不要在 ESM 验证文件里直接调用裸 `require`。
3. 每轮只补一个最小因果单元：一个值、一个函数壳、一个返回对象或一个对象契约。
4. `env.report()` 中 0 error / 0 undefined 只代表外部访问链看起来完整，不代表签名已可用。
5. 签名长度、前缀、编码形态和浏览器不一致时，继续定位 `first divergence`，不要直接迁移到根目录 `main.py`。
6. 如果有 Proxy / gap log，先用 `scripts/gap-log-module-advisor.js` 聚合推荐模块，再决定手写补丁顺序。
7. 如果需要真实浏览器种子，把 `scripts/browser-seed-collector.js` 作为 DevTools Snippet 执行，结果只落到当前任务的 `js_reverse_cache/`。
