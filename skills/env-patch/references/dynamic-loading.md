...
# 从 JSON 响应中提取 JS（跨平台 Python 写法）
python -c "import json, pathlib; data=json.loads(pathlib.Path('source/scripting_response.json').read_text(encoding='utf-8')); pathlib.Path('source/dynamic_sdk.js').write_text(data['data']['data'], encoding='utf-8')"
```
