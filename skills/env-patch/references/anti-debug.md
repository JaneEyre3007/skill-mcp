...
反调试代码不会直接写在源码里（那样太容易发现），而是通过**动态代码生成**注入；在瑞数 challenge 场景里，这些逻辑常混在 challenge 文档、自执行函数或外链 JS 解密后的执行链里：

| 注入方式 | 示例 |
|---------|------|
| `eval("debugger")` | 瑞数 challenge、OB 混淆 |
| `Function("debugger")()` | 瑞数 challenge、sojson |
...
| 站点/保护 | 反调试手段 | 关键 hook |
|-----------|-----------|----------|
| 瑞数 RS4/RS5 challenge 链 | eval + Function 动态生成 debugger | eval + Function |
