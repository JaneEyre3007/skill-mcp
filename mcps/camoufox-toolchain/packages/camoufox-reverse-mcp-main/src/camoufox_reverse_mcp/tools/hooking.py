...
async def _hook_function(
    function_path: str, hook_code: str, position: Literal["before", "after", "replace"], non_overridable: bool,
) -> dict:
    try:
        page = await browser_manager.get_active_page()
    ...
    parent[fn] = wrapper;{freeze_code}
            ...
            return error_response(f"Invalid position: {position}. Use 'before', 'after', or 'replace'.")
