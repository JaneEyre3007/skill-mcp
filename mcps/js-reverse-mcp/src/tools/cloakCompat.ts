 ...
 */
export function setCloakConfig(cloak: boolean | undefined, binaryPath: string | undefined): void {
  _cloakActive = !!cloak;
...
const CLOAK_CONFIG_FILE = path.resolve(
  import.meta.dirname,  // <mcp-root>/build/src/tools
  '../../..',           // → <mcp-root>
  'browser-connection',
  'config.json',
);
...
export const browserBinaryInfo = defineTool({
  ...
  handler: async (_request, response, context) => {
    ...
    let cloakBinaryPath: string | undefined = overrides?.cloak
    ...
    appendJson(response, {
      ...
      selected_url: context.getSelectedPage().url(),
      note:
        'cloak_active: true = CloakBrowser is currently running. ' +
...
export const launchBrowser = defineTool({
  ...
  schema: {
    ...
    cloakBinaryPath: zod.string().optional().describe(
      'Path to CloakBrowser executable. When provided, closes current browser and relaunches with CloakBrowser. ' +
