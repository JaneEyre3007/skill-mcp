...
import { exportNetworkRequestPart, getFormattedHeaderEntries, getFormattedResponseBody, getFormattedSetCookieEntries, getSetCookieFlowValues, getShortDescriptionForRequestAsync, getStatusFromRequestAsync, headersContainSensitiveValues, } from '../../src/formatters/networkFormatter.js';
...
test('keeps exact header values when redaction is disabled', () => {
    const lines = getFormattedHeaderEntries([{ name: 'Authorization', value: 'Bearer abc.def' }], { redactSensitiveValues: false });
    assert.deepEqual(lines, ['- Authorization:Bearer abc.def']);
});
test('does not treat Set-Cookie as a redacted generic header', () => {
    assert.equal(headersContainSensitiveValues([{ name: 'Set-Cookie', value: 'sid=abc' }]), false);
});
test('formats Set-Cookie entries as name=value and omits long values', () => {
    const longValue = 'x'.repeat(513);
    assert.deepEqual(getFormattedSetCookieEntries([
...
test('formats pending request list entries without waiting for a response', async () => {
    ...
    assert.equal(await getShortDescriptionForRequestAsync(request, 7, false, true), 'reqid=7 [time unavailable, pending] [xhr] POST https://example.test/api?a=1 [pending: resume execution before reading response data]');
});
test('formats pending request status without waiting for a response', async () => {
    const request = createPendingRequest();
    assert.equal(await getStatusFromRequestAsync(request), '[pending: resume execution before reading response data]');
});
test('rejects pending response exports without waiting for a response', async () => {
    const request = createPendingRequest();
    await assert.rejects(() => exportNetworkRequestPart(request, 'responseHeaders'), /Request is pending/);
    await assert.rejects(() => exportNetworkRequestPart(request, 'responseBody'), /Request is pending/);
    await assert.rejects(() => exportNetworkRequestPart(request, 'all'), /Request is pending/);
});
test('rejects an unknown outputPart instead of returning undefined', async () => {
    ...
    const request = createPendingRequest();
    await assert.rejects(() => exportNetworkRequestPart(request, 'bogus'), /Unknown outputPart/);
});
...
test('reports body eviction when neither cache nor live body is available', async () => {
    ...
    assert.equal(await getFormattedResponseBody(response), '<not available anymore — body evicted after navigation>');
});
