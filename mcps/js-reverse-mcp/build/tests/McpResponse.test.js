...
test('formats cookieName as complete Set-Cookie flow without pagination', async () => {
    ...
    const context = {
        ...
        getNetworkRequestStableId: (request) => ids.get(request) ?? 0,
    };
    ...
    assert.match(text, /Pagination ignored: Set-Cookie flow shows all matching updates in the current captured queue\./);
    ...
    assert.ok(text.indexOf('https://example.test/first') <
