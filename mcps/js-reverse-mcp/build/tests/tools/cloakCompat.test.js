...
test('AST instrumentation rewrites safe member reads only', () => {
    const result = rewriteInstrumentedSource(`const ua = navigator.userAgent;
    ...
    assert.match(result.source, /__mcp_tap_get\("probe","navigator\.userAgent",navigator\.userAgent\)/);
...
test('AST instrumentation falls back to scanner on parse errors', () => {
    const result = rewriteInstrumentedSource('const broken = ; navigator.userAgent;', {
        tag: 'probe',
        mode: 'ast',
        rewriteMemberAccess: true,
        maxRewrites: 20,
