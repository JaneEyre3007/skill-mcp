...
import { closeBrowser, ensureBrowserConnected, ensureBrowserLaunched, getRuntimeLaunchOverrides, } from './browser.js';
...
async function getContext() {
    ...
    if (args.browserUrl) {
        ...
        if (args.geoip) {
            try {
                ...
                if (geo.timezone && !args.timezone)
                    timezone = geo.timezone;
                if (geo.locale && !args.locale)
                    locale = geo.locale;
                logger(`GeoIP resolved: ip=${geo.ip} country=${geo.country} locale=${locale} timezone=${timezone}`);
            }
            catch (e) {
        ...
        result = await ensureBrowserLaunched({
            ...
            logFile,
            cloak: getRuntimeLaunchOverrides()?.cloak ?? args.cloak,
            cloakBinaryPath: getRuntimeLaunchOverrides()?.cloak
                ? getRuntimeLaunchOverrides().cloakBinaryPath
    ...
    cloakCompatTools.setHumanizeConfig(args.humanize ?? false, resolveHumanConfig((args.humanPreset ?? 'default')));
    // Propagate cloak settings so browser_binary_info can report the effective
    // cloak state (cloak_active / cloak_binary_path) without importing main.ts.
    cloakCompatTools.setCloakConfig(args.cloak, args.cloakBinaryPath);
    return context;
}
const logDisclaimers = () => {
    console.error(`js-reverse-mcp exposes content of the browser instance to the MCP clients allowing them to inspect,
...
function withToolTimeout(promise, timeoutMs, toolName) {
    ...
    const timeout = new Promise((_, reject) => {
        timeoutId = setTimeout(() => {
            reject(new Error(`Tool "${toolName}" timed out after ${timeoutMs}ms. If execution is paused at a breakpoint, call pause_or_resume and retry.`));
        }, timeoutMs);
    });
...
function registerTool(tool) {
    server.registerTool(tool.name, {
        ...
        try {
            ...
            })(), DEFAULT_TOOL_TIMEOUT_MS, tool.name);
        }
        catch (err) {
...
async function withShutdownTimeout(promise, reason) {
    let timeoutId;
    const timeout = new Promise(resolve => {
        timeoutId = setTimeout(() => {
            logger(`Shutdown cleanup timed out after ${SHUTDOWN_TIMEOUT_MS}ms: ${reason}`);
            resolve();
        }, SHUTDOWN_TIMEOUT_MS);
    });
...
function getStreamErrorCode(error) {
...
process.stdout.on('error', error => {
    ...
    requestShutdown(code === 'EPIPE' || code === 'ECONNRESET'
        ? `stdout ${code}`
        : `stdout error: ${getErrorText(error)}`, code === 'EPIPE' || code === 'ECONNRESET' ? 0 : 1);
});
