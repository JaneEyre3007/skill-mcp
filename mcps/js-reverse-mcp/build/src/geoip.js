...
async function fetchText(url, timeoutMs, dispatcher) {
 ...
 */
async function lookupIp(ip, timeoutMs = 5000, dispatcher) {
    try {
        const resText = await fetchText(`http://ip-api.com/json/${encodeURIComponent(ip)}?fields=status,countryCode,timezone`, timeoutMs, dispatcher);
 ...
 */
export async function resolveProxyGeo(proxyUrl, timeoutMs = 5000) {
 ...
 */
export async function resolveLocalGeo(timeoutMs = 5000) {
    for (const url of IP_ECHO_URLS) {
        try {
            const ip = (await fetchText(url, timeoutMs)).trim();
            if (/^[\d.]+$|^[0-9a-f:]+$/i.test(ip))
                return lookupIp(ip, timeoutMs);
        }
        catch {
            // try next
        }
