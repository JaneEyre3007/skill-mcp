...
async function loadCloakBrowser() {
    try {
        ...
        throw new Error('--cloak requires the `cloakbrowser` package. ' +
 ...
 */
async function withStdoutRedirectedToStderr(fn) {
...
function getOrCreateSeed(profileDir) {
    ...
    try {
        ...
        if (winnerSeed !== undefined)
            return winnerSeed;
        throw new Error(`Failed to read CloakBrowser fingerprint seed: ${seedFile}`);
    }
}
...
function waitForSeedFile(seedFile) {
 ...
 */
export async function setupCloak(profileDir, binaryPath, fingerprintSeed) {
...
async function resolveDownloadedBinaryPath() {
    ...
    return await withStdoutRedirectedToStderr(async () => {
        if (!cloak.binaryInfo().installed) {
            process.stderr.write('[js-reverse-mcp] Downloading CloakBrowser stealth binary (~200MB, one-time setup)...\n');
        }
