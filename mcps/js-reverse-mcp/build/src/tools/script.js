...
async function loadLocalFile(filePath) {
    if (filePath.startsWith('file://')) {
        throw new Error('localFilePath must be an absolute path, not a file:// URL.');
    }
    if (filePath.startsWith('~')) {
        throw new Error('localFilePath must be an absolute path; ~ is not expanded.');
    }
    if (!path.isAbsolute(filePath)) {
        throw new Error('localFilePath must be an absolute path.');
    }
    if (/[{}[\]*?]/.test(filePath)) {
        throw new Error('localFilePath must point to one file; globs are not supported.');
    }
    const resolvedPath = path.resolve(filePath);
    const stat = await fs.stat(resolvedPath).catch(error => {
        throw new Error(`Could not read localFilePath: ${error instanceof Error ? error.message : String(error)}`);
    });
    if (!stat.isFile()) {
        throw new Error('localFilePath must point to a regular file.');
    }
    if (stat.size > MAX_LOCAL_FILE_BYTES) {
        throw new Error(`localFilePath is too large (${stat.size} bytes). Maximum supported size is ${MAX_LOCAL_FILE_BYTES} bytes.`);
    }
...
`),
        ...
        frameIndex: zod
            ...
            .int()
            .optional()
            .describe('When paused at a breakpoint, which call frame to evaluate in (0 = top frame). ' +
            'If omitted, uses the top frame. Use get_paused_info to see available frames.'),
        outputFile: zod
            .string()
            .optional()
            .describe('If provided, saves the evaluation result to this local file path instead of returning it in the chat. JSON-serializable results are saved as JSON text; ArrayBuffer and Uint8Array results are saved as raw bytes. Useful for dumping large data or binary memory regions. The response reports the resolved absolute path.'),
        localFilePath: zod
            .string()
            .optional()
            .describe('Absolute path to one local file to pass to the evaluated function as localFile. Relative paths, file:// URLs, globs, ~, and directories are rejected. If provided, write the function as async ({ localFile }) => { ... }. Use localFile.text when present for UTF-8 text/JSON and localFile.base64 for exact bytes.'),
    },
    handler: async (request, response, context) => {
        const { function: fnString, mainWorld, frameIndex, outputFile, localFilePath, } = request.params;
        const localFile = localFilePath
            ? await loadLocalFile(localFilePath)
            : undefined;
        if (localFile) {
            response.appendResponseLine(`Loaded local file ${localFile.path} (${localFile.size} bytes).`);
        }
      ...
      try {
        ...
        const handleEvalResult = async (rawString) => {
            let parsed;
            try {
            ...
            if (parsed.type === 'base64') {
                ...
                if (truncated) {
                    response.appendResponseLine(`Result is ${data.length} chars; inline output is truncated to ${INLINE_EVAL_RESULT_LIMIT} chars. Re-run with outputFile to save the exact result.`);
                }
        ...
        if (debugger_.isEnabled() && debugger_.isPaused()) {
            if (localFile && localFile.size > MAX_PAUSED_LOCAL_FILE_BYTES) {
                throw new Error(`localFilePath is too large for paused call-frame evaluation (${localFile.size} bytes). Maximum supported paused size is ${MAX_PAUSED_LOCAL_FILE_BYTES} bytes.`);
            }
            ...
            if (frameIdx < 0 || frameIdx >= pausedState.callFrames.length) {
                throw new Error(`frameIndex ${frameIdx} is out of range (0-${pausedState.callFrames.length - 1})`);
            }
            const callFrameId = pausedState.callFrames[frameIdx]?.callFrameId;
            if (callFrameId) {
                const result = await debugger_.evaluateOnCallFrame(callFrameId, wrapResultSync(), { returnByValue: true });
                if (result.exceptionDetails) {
                    const errMsg = result.exceptionDetails.exception?.description ||
                        result.exceptionDetails.text;
                    throw new Error(`Script evaluation error: ${errMsg}`);
                }
                await handleEvalResult(result.result.value);
                return;
            }
        }
        ...
        if (mainWorld) {
            ...
            const result = await withTimeout(frame.evaluate(async ({ bridge, fn, id }) => {
          ...
          `;
            ...
            }, { bridge: CONSOLE_BRIDGE_SOURCE, fn: wrapResultAsync(), id: bridgeId }), DEFAULT_SCRIPT_TIMEOUT, 'Script evaluation timed out');
        ...
        try {
            const frame = context.getSelectedFrame();
            fnHandle = await withTimeout(frame.evaluateHandle(`(${wrapResultAsync()})`), DEFAULT_SCRIPT_TIMEOUT, 'Script evaluation timed out');
            await context.waitForEventsAfterAction(async () => {
                const result = await withTimeout(frame.evaluate(async (fn) => {
                    // @ts-expect-error no types.
                    return await fn();
                }, fnHandle), DEFAULT_SCRIPT_TIMEOUT, 'Script execution timed out');
                await handleEvalResult(result);
            });
        }
        finally {
