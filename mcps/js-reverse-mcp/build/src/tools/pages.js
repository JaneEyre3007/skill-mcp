...
export async function waitForNavigationOrPause(navigation, debugger_) {
    const navigationResult = navigation.then(() => ({ status: 'completed' }), error => ({ status: 'error', error }));
    if (!debugger_.isEnabled()) {
        return navigationResult;
    }
    let stopped = false;
    const pauseResult = (async () => {
        while (!stopped) {
...
export const selectPage = defineTool({
    ...
    schema: {
        pageIdx: zod
            .number()
            .optional()
            .describe('The index of the page to select. If omitted, lists all pages without changing selection.'),
    },
    ...
    handler: async (request, response, context) => {
        ...
        switch (request.params.type) {
            case 'url':
                ...
                {
                    const result = await waitForNavigationOrPause(page.goto(request.params.url, {
                        ...options,
                        waitUntil: 'domcontentloaded',
                        referer: DEFAULT_REFERER,
                    }), debugger_);
                    if (result.status === 'completed') {
                        ...
                        response.appendResponseLine(`Unable to navigate in the selected page: ${getErrorMessage(result.error)}.`);
                    }
                }
                break;
            case 'back':
                {
                    const result = await waitForNavigationOrPause(page.goBack({
                        ...options,
                        waitUntil: 'domcontentloaded',
                    }), debugger_);
                    if (result.status === 'completed') {
                        ...
                        response.appendResponseLine(`Unable to navigate back in the selected page: ${getErrorMessage(result.error)}.`);
                    }
                }
                break;
            case 'forward':
                {
                    const result = await waitForNavigationOrPause(page.goForward({
                        ...options,
                        waitUntil: 'domcontentloaded',
                    }), debugger_);
                    if (result.status === 'completed') {
                        ...
                        response.appendResponseLine(`Unable to navigate forward in the selected page: ${getErrorMessage(result.error)}.`);
                    }
                }
                break;
            case 'reload':
                {
                    const result = await waitForNavigationOrPause(page.reload({
                        ...options,
                        waitUntil: 'domcontentloaded',
                    }), debugger_);
                    if (result.status === 'completed') {
                        ...
                        response.appendResponseLine(`Unable to reload the selected page: ${getErrorMessage(result.error)}.`);
                    }
                }
                break;
        }
