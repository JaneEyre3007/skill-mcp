 ...
 */
async function appendStepSummary(response, debugger_, action, frame) {
    ...
    response.appendResponseLine(`${action} �?${shortUrl}:${line}:${col}, function ${funcName}`);
    // Show function arguments via evaluateOnCallFrame
    try {
        const argsResult = await debugger_.evaluateOnCallFrame(frame.callFrameId, `(() => { try { return JSON.stringify(Array.from(arguments)).slice(0, 500); } catch(e) { return String(arguments.length) + ' args'; } })()`, { returnByValue: true });
    ...
    try {
        ...
        if (lineContent) {
            ...
            response.appendResponseLine(`  > ${prefix}${lineContent.substring(s, e)}${suffix}`);
        }
    }
    catch {
        // Source unavailable
...
export const listScripts = defineTool({
    name: 'list_scripts',
    description: 'Lists all JavaScript scripts loaded in the current page. Returns script ID, URL, and source map information. Use this to find scripts before setting breakpoints or searching. Script IDs are valid for the current page load only; after navigation, call list_scripts again or prefer script URLs for follow-up tools.',
    annotations: {
        title: 'List Scripts',
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            if (bytecode) {
                ...
                response.appendResponseLine(`Script ${scriptId} is a WebAssembly binary file (${binaryData.length} bytes). Please use save_script_source to download it as a .wasm file.`);
                return;
            }
            ...
            if (startLine !== undefined || endLine !== undefined) {
                ...
                if (content.length > 1000) {
                    ...
                    response.appendResponseLine(`Recommended: use save_script_source to download the full file �?it auto-formats minified code so you can read it normally afterwards.`);
                    response.appendResponseLine(`Or use offset/length params for a partial read. The character offset for line ${start + 1} is ${lineOffset}.`);
            ...
            if (source.length > 1000) {
                response.appendResponseLine(`Script ${scriptId} is large (${source.length} chars). To view the whole file, use save_script_source (auto-formats minified code). To read a portion inline, use offset/length or startLine/endLine.`);
                response.appendResponseLine(`First 1000 characters:\n`);
                response.appendResponseLine('```javascript');
...
export const saveScriptSource = defineTool({
    ...
    schema: {
        ...
        filePath: zod
            .string()
            .describe('Local file path to save the script source to. Absolute paths and paths relative to the current working directory are supported. Use a .js/.mjs/.cjs/.jsx/.ts/.tsx extension to enable auto-format (prettier beautify); other extensions save raw source verbatim. For WASM scripts, use a .wasm extension.'),
        format: boolParam()
            .optional()
            .default(true)
            .describe('Auto-format JavaScript/TypeScript output with prettier (beautifies minified code). Defaults to true. Set to false to save the raw original source verbatim.'),
    },
    handler: async (request, response, context) => {
        const debugger_ = context.debuggerContext;
        if (!debugger_.isEnabled()) {
        ...
        try {
            ...
            if (url) {
                ...
                response.appendResponseLine(`Resolved URL to script ${resolvedId} (${result.script.url}).`);
            }
            else {
                const result = await debugger_.getScriptSource(scriptId);
                source = result.scriptSource;
            ...
            if (bytecode) {
                ...
                response.appendResponseLine(`Saved WASM script source to ${result.filename} (${binaryData.length} bytes).`);
            }
            else {
                let output = source;
                let formatNote = '';
                ...
                response.appendResponseLine(`Saved script source to ${result.filename} (${output.length} chars${formatNote}).`);
            }
        }
        catch (error) {
            response.appendResponseLine(`Error saving script source: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
});
/**
    ...
    handler: async (request, response, context) => {
        ...
        try {
            if (breakpointId) {
                ...
                const parts = [];
                if (codeCount > 0)
                    parts.push(`${codeCount} code`);
                if (xhrCount > 0)
                    parts.push(`${xhrCount} XHR`);
                response.appendResponseLine(`Removed ${parts.join(' + ')} breakpoint(s).`);
            }
            if (debugger_.isPaused()) {
                response.appendResponseLine('Execution is still paused. Use pause_or_resume when you are ready to continue.');
            }
        }
        catch (error) {
...
export const listBreakpoints = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        response.appendResponseLine(`Active breakpoints (${breakpoints.length} code, ${xhrBreakpoints.length} XHR/Fetch):\n`);
...
export const getPausedInfo = defineTool({
    ...
    schema: {
        ...
        frameIndex: zod
            ...
            .optional()
            .default(0)
            .describe('Which call frame to inspect scope variables for (0 = top frame). ' +
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.includeScopes && pausedState.callFrames.length > 0) {
            ...
            if (frameIndex < 0 || frameIndex >= pausedState.callFrames.length) {
                        ...
                        try {
                            ...
                            if (variables.length === 0) {
                                ...
                                for (const variable of variables.slice(0, 20)) {
                                    let valueStr = typeof variable.value === 'string'
                                        ? `"${variable.value}"`
                ...
                if (scopeCount === 0) {
                    response.appendResponseLine('    (no matching scopes �?try increasing maxScopeDepth)');
                }
            }
        }
...
export const step = defineTool({
    ...
    schema: {
        direction: zod
            .enum(['over', 'into', 'out'])
            .describe('Step direction: "over" (next statement), "into" (enter function), "out" (exit function).'),
    },
    handler: async (request, response, context) => {
        const debugger_ = context.debuggerContext;
        if (!debugger_.isEnabled()) {
        ...
        try {
            ...
            response.appendResponseLine(`Error stepping ${direction}: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
});
/**
...
export const setBreakpointOnText = defineTool({
    ...
    schema: {
        ...
        condition: zod
            .string()
            .optional()
            .describe('Optional synchronous JavaScript condition evaluated in the breakpoint call frame. Use only as a simple guard after choosing a precise code location; avoid complex logic, async work, or side effects. The breakpoint pauses only when this expression evaluates to true.'),
    },
    handler: async (request, response, context) => {
        const debugger_ = context.debuggerContext;
        if (!debugger_.isEnabled()) {
