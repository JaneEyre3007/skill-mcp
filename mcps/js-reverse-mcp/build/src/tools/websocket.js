...
export const getWebSocketMessages = defineTool({
    ...
    schema: {
        wsid: zod
            .number()
            .optional()
            .describe('The wsid of the WebSocket connection. If omitted, lists all connections.'),
        analyze: boolParam()
            .optional()
            .default(false)
            .describe('Set to true to analyze and group messages by pattern/fingerprint. Returns statistics and sample indices for each message type.'),
        frameIndex: zod
            .number()
            .int()
            .min(0)
            .optional()
            .describe('Get a single message by its raw frame index (0-based). Use the Idx values shown by message tables or the sample indices returned by analyze=true.'),
        direction: zod
        ...
        groupId: zod
            .string()
            .optional()
            .describe('Filter by group ID (A, B, C, ...). Run with analyze=true first to get group IDs; if analyze used a direction filter, pass the same direction here.'),
        pageSize: zod
            .number()
            .int()
            .positive()
        ...
        urlFilter: zod
            .string()
            .optional()
            .describe('Filter connections by URL (only for listing connections without wsid).'),
        includePreservedConnections: boolParam()
            .default(false)
            .optional()
            .describe('Set to true to return the preserved connections over the last 3 navigations (only for listing connections without wsid).'),
    },
    handler: async (request, response, context) => {
        ...
        const getFilteredFrames = () => {
            const frameIndices = ws.frames
                .map((_, index) => index)
                .filter(index => !request.params.direction ||
        ...
        if (request.params.analyze) {
            ...
            const summary = analyzeWebSocketFramesV2(frames, request.params.wsid, ws.connection.url, frameIndices);
            ...
            response.appendResponseLine(`- View single: \`get_websocket_messages(wsid=${request.params.wsid}, frameIndex=0)\``);
            return;
        }
        const { frames, frameIndices } = getFilteredFrames();
        ...
        if (request.params.groupId) {
            ...
            if (!summary) {
                summary = analyzeWebSocketFramesV2(frames, request.params.wsid, ws.connection.url, frameIndices);
                if (!request.params.direction) {
                    context.cacheTrafficSummary(request.params.wsid, summary);
                }
            }
