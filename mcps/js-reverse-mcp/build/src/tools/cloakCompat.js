...
import { defaultHumanConfig, newCursorState, humanClickSelector, humanTypeText, humanScroll as humanScrollAction, } from '../human.js';
import { zod } from '../third_party/index.js';
import { closeBrowser, getRuntimeLaunchOverrides, setRuntimeLaunchOverrides, } from '../browser.js';
import { ToolCategory } from './categories.js';
import { boolParam } from './paramHelpers.js';
import { defineTool } from './ToolDefinition.js';
const WAIT_UNTIL = ['load', 'domcontentloaded', 'networkidle'];
const REQUEST_ACTIONS = ['log', 'block', 'modify', 'mock', 'stop'];
const HOOK_POSITIONS = ['before', 'after', 'replace'];
const HOOK_MODES = ['intercept', 'trace'];
const STORAGE_TYPES = ['local', 'session'];
const COOKIE_ACTIONS = ['get', 'set', 'delete'];
const SCRIPT_ACTIONS = ['list', 'get', 'save'];
const persistentHookIds = new WeakMap();
const routeRegistry = new WeakMap();
const sourceMapCache = new WeakMap();
const cpuProfileSessions = new WeakMap();
const eventListenerBreakpoints = new WeakMap();
const cdpBreakpoints = new WeakMap();
...
const consoleWatermark = new WeakMap();
...
let _humanConfig = defaultHumanConfig();
/** Called by main.ts to propagate --humanize / --humanPreset settings. */
export function setHumanizeConfig(enabled, config) {
    _humanizeEnabled = enabled;
    _humanConfig = config;
}
// Module-level cloak config (set at server startup via setCloakConfig)
let _cloakActive = false;
let _cloakBinaryPath;
/**
 * Called by main.ts at startup to propagate --cloak / --cloakBinaryPath CLI settings
 * to the tools layer so browser_binary_info can return the effective cloak state
 * without a circular dependency on main.ts.
 */
export function setCloakConfig(cloak, binaryPath) {
    _cloakActive = !!cloak;
    _cloakBinaryPath = binaryPath;
}
function getCursorState(internal) {
...
`;
function asInternalContext(context) {
    return context;
}
export function rewriteInstrumentedSource(src, options) {
    if (options.mode === 'regex') {
        const [source, edits] = scannerRewrite(src, options.tag, options.rewriteMemberAccess, options.maxRewrites, options.filterPropertyNames, options.filterObjectNames);
...
function astRewrite(src, options) {
    ...
    traverseAst(ast, {
        MemberExpression(path) {
            if (edits >= options.maxRewrites)
                return;
            if (!isSafeMemberRead(path))
                return;
            const info = memberAccessInfo(path.node, src);
            if (!info)
                return;
            if (objFilter.size > 0 && !objFilter.has(info.objectName))
                return;
            if (propFilter.size > 0 &&
                (!info.propertyName || !propFilter.has(info.propertyName))) {
                return;
            }
            const replacement = t.callExpression(t.memberExpression(t.identifier('window'), t.identifier('__mcp_tap_get')), [
...
function memberAccessInfo(node, src) {
    ...
    const key = nodeStart === null || nodeEnd === null
        ? propertyName
...
function isSafeMemberRead(path) {
    if ('optional' in path.node && path.node.optional)
        return false;
    if (isWithinAssignmentTarget(path))
        return false;
...
function isWithinAssignmentTarget(path) {
...
function nodeContains(container, child) {
    if (container === child)
        return true;
...
function scannerRewrite(src, tag, rewriteMemberAccess = true, maxRewrites = 20000, filterPropertyNames, filterObjectNames) {
    let edits = 0;
    const propFilter = new Set(filterPropertyNames ?? []);
    const objFilter = new Set(filterObjectNames ?? []);
    if (!rewriteMemberAccess)
        return [INSTRUMENT_RUNTIME + '\n' + src, edits];
    const chunks = [];
    let last = 0;
    let i = 0;
    while (i < src.length) {
        ...
        chunks.push(`window.__mcp_tap_get(${JSON.stringify(tag)},${JSON.stringify(`${obj}.${prop}`)},${obj}.${prop})`);
        last = propEnd;
        edits++;
        i = propEnd;
    }
...
function readIdentifierEnd(src, index) {
    while (index < src.length && isIdentifierPart(src[index]))
        index++;
    return index;
}
function nextNonSpace(src, index) {
    while (index < src.length && /\s/.test(src[index]))
        index++;
    return index;
}
function prevNonSpace(src, index) {
    while (index >= 0 && /\s/.test(src[index]))
        index--;
    return index >= 0 ? src[index] : '';
}
function isAssignmentOrUpdate(src, index) {
    const ch = src[index];
...
function skipLineComment(src, index) {
    const end = src.indexOf('\n', index);
    return end === -1 ? src.length : end + 1;
}
function skipBlockComment(src, index) {
    const end = src.indexOf('*/', index);
    return end === -1 ? src.length : end + 2;
}
function skipQuotedString(src, index, quote) {
...
function skipTemplateLiteral(src, index) {
    while (index < src.length) {
        if (src[index] === '\\') {
            index += 2;
            continue;
...
function looksLikeRegexStart(src, index) {
...
function jsvmpTransparentHookSource(maxEntries) {
    return `(() => {
...
function appendJson(response, value) {
    response.appendResponseLine('```json');
    response.appendResponseLine(JSON.stringify(value, null, 2));
    response.appendResponseLine('```');
}
function appendUnsupportedPersistentClear(response, count) {
    appendJson(response, {
        status: 'unsupported',
...
async function getCdpSession(context) {
    return context.sessionProvider.getSession(context.getSelectedPage());
}
async function evaluateMainWorld(page, source) {
...
async function addPersistentMainWorldScript(context, name, source) {
...
async function getScriptSourceByUrlOrId(context, url, scriptId) {
...
function sourceSlice(source, startLine, endLine, offset, length = 1000) {
    if (offset !== undefined) {
...
function hookPresetSource(preset) {
    const base = `
(() => {
  const root = window;
...
export const navigate = defineTool({
    name: 'navigate',
    description: 'Navigate to a URL with optional hook pre-injection and redirect tracing.',
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.pre_inject_hooks) {
            for (const preset of request.params.pre_inject_hooks) {
                ...
                await addPersistentMainWorldScript(asInternalContext(context), `preset:${preset}`, source);
...
export const click = defineTool({
    name: 'click',
    description: 'Click a page element by selector. Uses human-like cursor movement when --humanize is enabled.',
    annotations: { category: ToolCategory.NAVIGATION, readOnlyHint: false },
    ...
    handler: async (request, response, context) => {
        ...
        if (_humanizeEnabled) {
            ...
            await humanClickSelector(page, state, request.params.selector, _humanConfig);
        }
        else {
...
export const typeText = defineTool({
    name: 'type_text',
    description: 'Type text into an input field. Uses variable delays and pauses when --humanize is enabled.',
    annotations: { category: ToolCategory.NAVIGATION, readOnlyHint: false },
    schema: {
        selector: zod.string(),
        text: zod.string(),
...
export const humanScroll = defineTool({
    name: 'human_scroll',
    description: 'Scroll the selected page. Uses burst-chunk scrolling with settle delay when --humanize is enabled.',
    annotations: { category: ToolCategory.NAVIGATION, readOnlyHint: false },
...
export const getConsoleLogs = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        let logs = visibleItems.map(item => {
            if ('type' in item) {
                return {
                    level: item.type(),
                    text: item.text(),
        ...
        if (request.params.clear)
            consoleWatermark.set(internal, allItems.length);
        appendJson(response, {
            logs,
            total: logs.length,
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.action === 'get') {
            ...
            if (request.params.domain)
                all = all.filter(cookie => cookie.domain.includes(request.params.domain));
        ...
        if (request.params.action === 'set') {
            ...
            await browserContext.addCookies(list);
...
export const getStorage = defineTool({
    ...
    handler: async (request, response, context) => {
        const storageName = request.params.storage_type === 'session'
            ? 'sessionStorage'
            : 'localStorage';
...
export const importState = defineTool({
    name: 'import_state',
    description: 'Import cookies and register localStorage seed script from a Playwright storage_state file.',
    annotations: { category: ToolCategory.BROWSER_STATE, readOnlyHint: false },
...
export const networkCapture = defineTool({
    ...
    description: 'Start/stop/clear/status network capture. js-reverse-mcp captures continuously; start/stop are compatibility toggles.',
...
export const getNetworkRequest = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        const snapshot = JSON.parse(Buffer.from(exported.data).toString('utf8'));
...
export const interceptRequest = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        const handler = async (route) => {
            if (request.params.action === 'block')
                return route.abort();
            if (request.params.action === 'mock')
                return route.fulfill(request.params.mock_response ?? { status: 200, body: '' });
        ...
        await page.route(request.params.url_pattern, handler);
...
export const evaluateJs = defineTool({
    ...
    handler: async (request, response, context) => {
        try {
            ...
            const value = await context.getSelectedFrame().evaluate(async ({ expression, awaitPromise }) => {
...
export const scripts = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.action === 'list') {
            appendJson(response, debugger_.getScripts().map(script => ({
        ...
        const source = await getScriptSourceByUrlOrId(asInternalContext(context), request.params.url, undefined);
        if (request.params.action === 'get') {
            appendJson(response, {
                url: source.url,
...
export const searchCode = defineTool({
    ...
    handler: async (request, response, context) => {
        const matches = await context.debuggerContext.searchInScripts(request.params.keyword, { caseSensitive: true, isRegex: false });
        let items = matches.matches;
        if (request.params.script_url)
            items = items.filter(item => item.url.includes(request.params.script_url));
        appendJson(response, {
...
export const hookFunction = defineTool({
    name: 'hook_function',
    description: 'Hook or trace a function path such as JSON.stringify. ' +
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.persistent)
            await addPersistentMainWorldScript(asInternalContext(context), `hook:${request.params.function_path}`, source);
        await evaluateMainWorld(context.getSelectedPage(), source);
        appendJson(response, {
...
export const injectHookPreset = defineTool({
    ...
    description: 'Inject preset hook: xhr/fetch/crypto/websocket/debugger_bypass/cookie/runtime_probe.',
    annotations: {
        category: ToolCategory.REVERSE_ENGINEERING,
        readOnlyHint: false,
    },
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.persistent)
            await addPersistentMainWorldScript(asInternalContext(context), `preset:${request.params.preset}`, source);
        await evaluateMainWorld(context.getSelectedPage(), source);
        appendJson(response, {
            status: 'injected',
...
export const removeHooks = defineTool({
    name: 'remove_hooks',
    description: 'Restore in-page function hooks when originals are tracked. Persistent init scripts cannot be removed from an existing Playwright context.',
    annotations: {
        category: ToolCategory.REVERSE_ENGINEERING,
        readOnlyHint: false,
    },
    ...
    handler: async (request, response, context) => {
        await evaluateMainWorld(context.getSelectedPage(), `(() => { if (!window.__mcp_hooks) return; for (const path of Object.keys(window.__mcp_hooks)) { const parts = path.split('.'); let parent = window; for (let i=0;i<parts.length-1;i++) parent = parent?.[parts[i]]; if (parent) parent[parts[parts.length-1]] = window.__mcp_hooks[path]; } window.__mcp_hooks = {}; })();`);
        const internal = asInternalContext(context);
...
export const compareEnv = defineTool({
    name: 'compare_env',
    description: 'Collect browser fingerprint environment values. Returns navigator, screen, webgl, timing and custom expression results.',
    annotations: { category: ToolCategory.DEBUGGING, readOnlyHint: true },
        ...
        const value = await context.getSelectedPage().evaluate(customProps => {
            ...
            for (const p of [
                ...
                try {
                    ...
                    result.navigator[p] = { error: e.message };
                }
            }
            const scr = screen;
            for (const p of [
                ...
                try {
                    ...
                    result.screen[p] = { error: e.message };
                }
            }
            ...
            for (const expr of customProps || []) {
                try {
                    ...
                    result.custom[expr] = { error: e.message };
                }
            }
            return result;
...
export const setCdpSkipAllPauses = defineTool({
    ...
    description: 'Tell Chromium Debugger to ignore debugger statements and breakpoints.',
    annotations: { category: ToolCategory.DEBUGGING, readOnlyHint: false },
    schema: {
...
export const setEventListenerBreakpoint = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (!set)
            eventListenerBreakpoints.set(asInternalContext(context), (set = new Set()));
...
export const listCdpScripts = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.url_filter)
            scripts = scripts.filter(script => script.url.includes(request.params.url_filter));
...
export const getCdpScriptSource = defineTool({
    ...
    handler: async (request, response, context) => {
        const result = await context.debuggerContext.getScriptSource(request.params.script_id);
...
export const getCdpSource = defineTool({
    ...
    handler: async (request, response, context) => {
        const source = await getScriptSourceByUrlOrId(asInternalContext(context), request.params.url ?? undefined, request.params.script_id ?? undefined);
        appendJson(response, {
            script_id: source.scriptId,
            url: source.url,
            source: sourceSlice(source.source, request.params.start_line ?? undefined, request.params.end_line ?? undefined, request.params.offset ?? undefined, request.params.length),
        });
    },
});
...
export const searchCdpSources = defineTool({
    ...
    handler: async (request, response, context) => {
        const result = await context.debuggerContext.searchInScripts(request.params.query, {
            caseSensitive: request.params.case_sensitive,
            isRegex: request.params.is_regex,
        });
        let matches = result.matches;
        if (request.params.url_filter)
            matches = matches.filter(match => match.url.includes(request.params.url_filter));
...
export const setCdpBreakpoint = defineTool({
    ...
    description: 'Set a breakpoint by script URL/scriptId and zero-based line/column.',
    annotations: {
        category: ToolCategory.REVERSE_ENGINEERING,
        readOnlyHint: false,
    },
    ...
    handler: async (request, response, context) => {
        ...
        if (!url)
            throw new Error('url is required when script_id has no URL');
        const bp = await context.debuggerContext.setBreakpoint(url, request.params.line_number, request.params.column_number, request.params.condition || undefined);
        let map = cdpBreakpoints.get(asInternalContext(context));
        if (!map)
            cdpBreakpoints.set(asInternalContext(context), (map = new Map()));
...
export const listCdpBreakpoints = defineTool({
    ...
    annotations: { category: ToolCategory.REVERSE_ENGINEERING, readOnlyHint: true },
    schema: {},
    handler: async (_request, response, context) => appendJson(response, context.debuggerContext.getBreakpoints()),
});
export const removeCdpBreakpoint = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        await context.debuggerContext.removeBreakpoint(request.params.breakpoint_id);
        appendJson(response, {
            status: 'removed',
...
export const stepDebugger = defineTool({
    ...
    handler: async (request, response, context) => {
        const frame = request.params.direction === 'into'
...
export const evaluateOnPaused = defineTool({
    ...
    handler: async (request, response, context) => {
        const frame = context.debuggerContext.getPausedState().callFrames[request.params.frame_index];
        if (!frame)
            throw new Error('frame_index out of range or execution is not paused');
        appendJson(response, await context.debuggerContext.evaluateOnCallFrame(frame.callFrameId, request.params.expression, { returnByValue: true }));
    },
});
...
export const stopCpuProfile = defineTool({
    ...
    handler: async (request, response, context) => {
        const session = cpuProfileSessions.get(asInternalContext(context)) ??
...
export const captureHeapSnapshot = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        const file = await context.saveFile(new TextEncoder().encode(chunks.join('')), request.params.save_path);
        appendJson(response, {
            status: 'saved',
            path: file.filename,
...
export const listSourceMaps = defineTool({
    ...
    handler: async (request, response, context) => {
        appendJson(response, context.debuggerContext
            .getScripts()
...
export const getSourceMap = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (!script?.sourceMapURL)
            throw new Error('script has no sourceMapURL');
        const map = await context.getSelectedPage().evaluate(async ({ mapUrl, scriptUrl }) => {
        ...
        if (!cache)
            sourceMapCache.set(asInternalContext(context), (cache = new Map()));
...
export const getSourceMapSource = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (!map) {
            ...
            map = await context
                .getSelectedPage()
                .evaluate(async ({ mapUrl, scriptUrl }) => fetch(new URL(mapUrl, scriptUrl || location.href).href).then(r => r.json()), { mapUrl: script.sourceMapURL, scriptUrl: script.url });
        }
        const loadedMap = map;
        const sources = loadedMap.sources ?? [];
        const sourcesContent = loadedMap.sourcesContent ?? [];
        const index = request.params.source_index ??
...
export const verifySignerOffline = defineTool({
    ...
    handler: async (request, response, context) => {
        const result = await context.getSelectedPage().evaluate(({ code, samples, compareParams }) => {
            const signer = (0, eval)(`(${code})`);
            const details = samples.map(sample => {
                const computed = signer(sample);
                const expected = sample.expected || {};
                const keys = compareParams || Object.keys(expected);
                const passed = keys.every(key => String(computed?.[key]) === String(expected?.[key]));
...
export const websocketCapture = defineTool({
    ...
    description: 'Enable, clear, or inspect CDP WebSocket capture. Capture is initialized lazily by existing collectors.',
    annotations: { category: ToolCategory.NETWORK, readOnlyHint: false },
    schema: {
...
export const listWebsockets = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.url_filter)
            connections = connections.filter(ws => ws.connection.url.includes(request.params.url_filter));
        appendJson(response, connections.map(ws => ({
...
/** Absolute path to the persistent CloakBrowser config file.
 *  Skills write the user-provided cloakBinaryPath here on first use so it
 *  survives across sessions without requiring --cloakBinaryPath at startup. */
const CLOAK_CONFIG_FILE = path.resolve(import.meta.dirname, // <mcp-root>/build/src/tools
'../../..', // → <mcp-root>
'browser-connection', 'config.json');
export const browserBinaryInfo = defineTool({
    ...
    handler: async (_request, response, context) => {
        ...
        const cloakActive = !!(overrides?.cloak ?? _cloakActive);
        let cloakBinaryPath = overrides?.cloak
        ...
        if (!cloakBinaryPath) {
            try {
                const raw = await fs.readFile(CLOAK_CONFIG_FILE, 'utf8');
                const saved = JSON.parse(raw);
                if (saved?.cloakBinaryPath && typeof saved.cloakBinaryPath === 'string') {
                    cloakBinaryPath = saved.cloakBinaryPath;
                }
            }
            catch {
                /* file absent or malformed — that is fine, treat as unconfigured */
            }
        }
        appendJson(response, {
            mode: 'patchright',
            ...
            config_file: CLOAK_CONFIG_FILE,
            selected_url: context.getSelectedPage().url(),
            note: 'cloak_active: true = CloakBrowser is currently running. ' +
                'cloak_binary_path: resolved CloakBrowser executable path (CLI > saved config > null). ' +
                'config_file: write {"cloakBinaryPath":"<path>"} here to persist the path across sessions. ' +
                'Switch at runtime via launch_browser({cloakBinaryPath: "<path>"}) or back with launch_browser({cloakBinaryPath: ""}).',
        });
    },
});
export const checkBrowserUpdate = defineTool({
    name: 'check_browser_update',
    description: 'Compatibility placeholder for CloakBrowser binary update checks.',
    annotations: { category: ToolCategory.DEBUGGING, readOnlyHint: true },
    schema: {},
    handler: async (_request, response) => appendJson(response, {
...
export const tracePropertyAccess = defineTool({
    ...
    description: 'Engine-level DOM property tracing is not available in Chromium/Patchright. Returns environment snapshot fallback.',
    annotations: { category: ToolCategory.DEBUGGING, readOnlyHint: true },
    schema: {
...
export const launchBrowser = defineTool({
    name: 'launch_browser',
    description: 'Launch (or switch) the browser for JS reverse engineering. ' +
        'Without cloakBinaryPath: reports current browser state. ' +
        'With cloakBinaryPath: closes the current browser and relaunches with CloakBrowser stealth Chromium. ' +
        'With cloakBinaryPath set to empty string: switches back to normal Chrome.',
    annotations: { category: ToolCategory.NAVIGATION, readOnlyHint: false },
    schema: {
        ...
        cloakBinaryPath: zod.string().optional().describe('Path to CloakBrowser executable. When provided, closes current browser and relaunches with CloakBrowser. ' +
            'Pass an empty string to switch back to normal Chrome. ' +
            'Omit to keep the current browser and only return its state.'),
    },
    handler: async (request, response, context) => {
        ...
        if (request.params.cloakBinaryPath !== undefined) {
            if (request.params.cloakBinaryPath === '') {
                // Switch back to normal Chrome
                setRuntimeLaunchOverrides(undefined);
                await closeBrowser('manual close via launch_browser (normal Chrome)');
                appendJson(response, {
                    status: 'switching',
                    target: 'normal Chrome',
                    note: 'Browser closed. Next tool call will automatically relaunch with system Chrome.',
                });
                return;
            }
            // Switch to CloakBrowser
            setRuntimeLaunchOverrides({
                cloak: true,
                cloakBinaryPath: request.params.cloakBinaryPath,
            });
            await closeBrowser('manual close via launch_browser (CloakBrowser)');
            appendJson(response, {
                status: 'switching',
                target: 'CloakBrowser',
                path: request.params.cloakBinaryPath,
                note: 'Browser closed. Next tool call will automatically relaunch with CloakBrowser.',
            });
            return;
        }
        appendJson(response, {
            status: 'already_running',
            note: 'js-reverse-mcp starts the browser at server startup. Pass cloakBinaryPath to switch between normal Chrome and CloakBrowser at runtime.',
            cli_flag_mapping: {
                headless: '--headless',
                locale: '--locale zh-CN',
...
export const closeBrowserTool = defineTool({
    name: 'close_browser',
    description: 'Close the managed browser and release all resources, clearing any runtime CloakBrowser override. The next tool call will automatically relaunch with the default (normal Chrome) browser.',
    annotations: { category: ToolCategory.NAVIGATION, readOnlyHint: false },
    schema: {},
    handler: async (_request, response) => {
        ...
        appendJson(response, {
            status: 'closed',
            note: 'Browser closed and runtime overrides cleared. Next tool call will launch normal Chrome.',
        });
    },
});
...
export const listCdpNetworkRequests = defineTool({
    ...
    description: 'List network requests with CDP-compatible naming, backed by the existing js-reverse-mcp network queue.',
    annotations: { category: ToolCategory.NETWORK, readOnlyHint: true },
    schema: {
    ...
    handler: async (request, response, context) => {
        ...
        requests = requests.filter(r => !r.url().includes('patchright-init-script-inject.internal'));
        if (request.params.url_filter) {
            requests = requests.filter(item => item.url().includes(request.params.url_filter));
        }
        if (request.params.method) {
            requests = requests.filter(item => item.method().toUpperCase() === request.params.method.toUpperCase());
        }
        if (request.params.resource_type) {
            requests = requests.filter(item => item.resourceType() === request.params.resource_type);
        }
        if (request.params.has_initiator !== undefined &&
            request.params.has_initiator !== null) {
            requests = requests.filter(item => Boolean(context.getRequestInitiator(item)) ===
                request.params.has_initiator);
        }
        const rows = await Promise.all(requests.slice(-request.params.limit).map(async (item) => {
...
function getRequestByCdpCompatId(context, cdpRequestId) {
    const numeric = Number(cdpRequestId);
    if (!Number.isInteger(numeric)) {
        throw new Error('This compatibility layer uses js-reverse request ids as cdp_request_id values. Pass the cdp_request_id returned by list_cdp_network_requests.');
    }
    return context.getNetworkRequestById(numeric);
}
export const getCdpNetworkRequest = defineTool({
    ...
    handler: async (request, response, context) => {
        const req = getRequestByCdpCompatId(asInternalContext(context), request.params.cdp_request_id);
        const exported = await exportNetworkRequestPart(req, 'all');
        const snapshot = JSON.parse(Buffer.from(exported.data).toString('utf8'));
        if (!request.params.include_body) {
            delete snapshot.requestBody;
...
export const getCdpRequestPostData = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        appendJson(response, {
            ...
            post_data: postData.length > request.params.max_size
...
export const hookJsvmpInterpreter = defineTool({
    ...
    description: 'Install a page-level JSVMP runtime probe. ' +
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.persistent) {
            await addPersistentMainWorldScript(asInternalContext(context), hookKey, source);
        }
        await evaluateMainWorld(context.getSelectedPage(), source);
        const warnings = [];
        if (context.getSelectedPage().url() !== 'about:blank')
            warnings.push('Hook installed on an already-loaded page; reload for best coverage.');
        if (mode === 'proxy')
...
export const instrumentation = defineTool({
    ...
    description: 'Source-level script instrumentation via route-based AST rewriting. ' +
    ...
    handler: async (request, response, context) => {
        ...
        if (request.params.action === 'install') {
            ...
            const routeHandler = async (route) => {
                ...
                const selectiveOversizedSkip = oversized &&
                ...
                if (oversized && (onOversized === 'skip' || selectiveOversizedSkip)) {
                    ...
                    const headers = Object.fromEntries(Object.entries(resp.headers()).filter(([k]) => !STRIP_HEADERS.has(k.toLowerCase())));
                ...
                const rewritten = rewriteInstrumentedSource(bodyStr, {
                    ...
                    rewriteMemberAccess,
                    maxRewrites,
                    filterPropertyNames: filterProps,
                    filterObjectNames: filterObjs,
                    fallbackOnError,
                });
                cache.set(reqUrl, rewritten.source);
                stats.files_rewritten = stats.files_rewritten + 1;
                stats.total_edits = stats.total_edits + rewritten.edits;
                stats.last_mode = rewritten.mode;
                if (rewritten.fallbackReason) {
                    stats.ast_fallbacks = stats.ast_fallbacks + 1;
                    stats.last_fallback_reason = rewritten.fallbackReason;
                }
                stats.last_url = reqUrl;
                const outHeaders = Object.fromEntries(Object.entries(resp.headers()).filter(([k]) => !STRIP_HEADERS.has(k.toLowerCase())));
            ...
            await asInternalContext(context).browserContext.route(pattern, routeHandler);
        ...
        if (request.params.action === 'log') {
            ...
            if (request.params.key_filter)
                logs = logs.filter(x => String(x['key'] ?? '').includes(request.params.key_filter));
            if (request.params.clear)
                await page.evaluate(() => {
                    const t = window;
        ...
        if (request.params.action === 'stop') {
            ...
            if (!routes)
                return appendJson(response, { status: 'stopped', count: 0 });
            ...
            for (const pat of targets) {
                ...
                if (!entry)
                    continue; // skip patterns not registered by us
                ...
                await asInternalContext(context)
                    .browserContext.unroute(pat, entry.handler)
                    .catch(() => undefined);
                routes.delete(pat);
                stopped++;
            }
...
export const listTraceFiles = defineTool({
    ...
    description: 'List trace files. Engine-level trace files are not produced by this MCP.',
    annotations: { category: ToolCategory.DEBUGGING, readOnlyHint: true },
    schema: { limit: zod.number().int().optional().default(20) },
    handler: async (_request, response) => appendJson(response, { traces_dir: null, total: 0, files: [] }),
});
export const queryTraceFile = defineTool({
    name: 'query_trace_file',
    description: 'Query a trace file. Engine-level trace files are not produced by this MCP.',
    annotations: { category: ToolCategory.DEBUGGING, readOnlyHint: true },
    schema: {
        file_path: zod.string(),
        mode: zod.string().optional().default('summary'),
    },
    handler: async (_request, response) => appendJson(response, { unsupported: true, entries: [] }),
});
