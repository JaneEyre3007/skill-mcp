...
function formatBridgeMessage(log) {
    const text = log.text.length > 1000 ? `${log.text.slice(0, 1000)}...` : log.text;
...
export const listConsoleMessages = defineTool({
    ...
    schema: {
        ...
        types: zod
            ...
            .optional()
            .describe('Filter messages to only return messages of the specified console message types. When omitted or empty, returns all messages.'),
    ...
    handler: async (request, response, context) => {
        if (request.params.msgid !== undefined) {
            const [bridgeLog] = await readConsoleBridgeLogs(context.getSelectedPage(), {
                id: request.params.msgid,
            });
            if (bridgeLog) {
                response.appendResponseLine(`ID: ${bridgeLog.id}`);
                response.appendResponseLine(`Message: ${bridgeLog.level}> ${bridgeLog.text}`);
