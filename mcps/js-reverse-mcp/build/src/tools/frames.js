...
export const selectFrame = defineTool({
    name: 'select_frame',
    description: 'Lists all frames (including iframes) in the current page. Pass frameIdx to switch the execution context used by evaluate_script. Other tools may still operate at page level.',
    annotations: {
        title: 'Select Frame',
    ...
    schema: {
        frameIdx: zod
            ...
            .min(0)
            .optional()
            .describe('The frame index to select. 0 = main frame. If omitted, lists all frames without changing selection.'),
    },
    handler: async (request, response, context) => {
        ...
        if (frameIdx >= frames.length) {
            response.appendResponseLine(`Invalid frame index ${frameIdx}. Available: 0-${frames.length - 1}.`);
            return;
        }
