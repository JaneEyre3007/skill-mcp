...
export const listNetworkRequests = defineTool({
    ...
    schema: {
        reqid: zod
            .preprocess(value => (value === 0 ? undefined : value), zod.number().int().positive().optional())
            .optional()
            .describe('The reqid of a specific network request to get full details for. If omitted, lists all requests.'),
        pageSize: zod
            .number()
            .int()
            .positive()
            .optional()
            .describe('Maximum number of requests to return for the normal network list. Defaults to 20. Ignored when cookieName is provided because Set-Cookie flow returns all matching captured updates.'),
        pageIdx: zod
            .number()
            .int()
            .min(0)
            .optional()
            .describe('Page number to return for the normal network list (0-based). When omitted, returns the first page. Ignored when cookieName is provided because Set-Cookie flow returns all matching captured updates.'),
        methods: zod
        ...
        resourceTypes: zod
            ...
            .optional()
            .describe('Filter requests to only return requests of the specified resource types (xhr, fetch, document, script, ...). This is the resource category, NOT the HTTP verb — use methods for GET/POST filtering. When omitted or empty, returns all requests.'),
        urlFilter: zod
            .string()
            .optional()
            .describe('Filter requests by URL. Only requests containing this substring will be returned.'),
        cookieName: zod
            .preprocess(value => typeof value === 'string' && value.trim() === '' ? undefined : value, zod.string().trim().min(1).optional())
            .optional()
            .describe('Switch to Set-Cookie flow mode for an exact response cookie name. Returns all currently captured responses that set/update this cookie, oldest-first, and shows the target cookie name=value for each update. Do not pass pageSize/pageIdx with cookieName; Set-Cookie flow ignores pagination and returns all matching captured updates. Does not match request Cookie headers.'),
        outputFile: zod
            .string()
            .optional()
            .describe('When reqid is provided, save network data to this local file instead of returning only inline text. Use this for exact bytes, large bodies, long GET query payloads, binary responses, replay/signature inputs, or data that will be decoded with external tools. Absolute paths and paths relative to the current working directory are supported. The response reports the resolved absolute path; use that path with evaluate_script localFilePath when browser-side processing is needed.'),
        outputPart: zod
            .enum(NETWORK_EXPORT_PARTS)
            .default('all')
            .describe('Which part to export when outputFile is provided. "responseHeaders" saves response headers as JSON while preserving repeated headers such as Set-Cookie, "responseBody" saves raw response bytes, "requestBody" saves captured request body bytes, "queryParams" saves parsed URL query parameters as JSON, and "all" saves a JSON bundle with metadata, headers, query params, and body content/metadata. Defaults to "all".'),
    },
    handler: async (request, response, context) => {
        if (request.params.outputFile && request.params.reqid === undefined) {
            response.appendResponseLine('outputFile requires reqid. First call list_network_requests without outputFile to find the request id, then re-run with reqid and outputFile.');
            return;
        }
        if (request.params.reqid !== undefined) {
            if (request.params.outputFile) {
                const networkRequest = context.getNetworkRequestById(request.params.reqid);
                const outputPart = request.params.outputPart;
                const exported = await exportNetworkRequestPart(networkRequest, outputPart);
                const file = await context.saveFile(exported.data, request.params.outputFile);
                response.appendResponseLine(`${exported.summary} Saved ${outputPart} to ${file.filename}.`);
                return;
            }
...
export const clearNetworkRequests = defineTool({
    ...
    handler: async (_request, response, context) => {
        ...
        response.appendResponseLine(`Cleared ${requestCount} network request${requestCount === 1 ? '' : 's'} for the selected page.`);
        response.appendResponseLine(`Released ${reclaimedBytes} bytes of cached response bodies.`);
        response.appendResponseLine('Request initiator data cleared.');
        response.appendResponseLine('reqids are not reused — newly collected requests continue from the previous high-water mark.');
    },
});
