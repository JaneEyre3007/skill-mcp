...
export async function installConsoleBridge(context) {
    await Promise.all(context.pages().map(page => ensureConsoleBridge(page)));
}
export async function ensureConsoleBridge(target) {
    await target.evaluate(CONSOLE_BRIDGE_SOURCE).catch(() => undefined);
}
export async function readConsoleBridgeLogs(page, options = {}) {
    ...
    const isolatedLogs = await page
        ...
        const logs = Array.isArray(target.__mcpConsoleLogs)
            ...
            : [];
        const result = id === undefined ? [...logs] : logs.filter(log => log.id === id);
        if (clear)
            logs.length = 0;
        return result;
    }, options)
...
async function readMainWorldConsoleBridgeLogs(page, options) {
    return page
        .evaluate(({ clear, id }) => {
