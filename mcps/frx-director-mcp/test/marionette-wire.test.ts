...
function startMock(onCommand?: (name: string, params: { args?: unknown[] }) => void | Promise<void>): Promise<{ port: number; server: net.Server }> {
  const server = net.createServer((sock) => {
    ...
    sock.on("data", (d) => {
      ...
      for (;;) {
        ...
        Promise.resolve(onCommand?.(name, params)).finally(() => {
  ...
  return new Promise((res) =>
    server.listen(0, "127.0.0.1", () => {
      servers.push(server);
      res({ port: (server.address() as net.AddressInfo).port, server });
    }),
  );
}
...
describe("MarionetteWire framing", () => {
  ...
  it("serializes concurrent commands so response frames cannot cross", async () => {
    let executeInFlight = 0;
    let maxExecuteInFlight = 0;
    const { port } = await startMock(async (name) => {
      if (name !== "WebDriver:ExecuteScript") return;
      executeInFlight += 1;
      maxExecuteInFlight = Math.max(maxExecuteInFlight, executeInFlight);
    ...
    const w = new MarionetteWire();
    await w.connect("127.0.0.1", port, 5000);
    const [a, b] = await Promise.all([w.execute("return 1;", ["a"]), w.execute("return 2;", ["b"])]);

    expect(a).toEqual({ echo: ["a"] });
    expect(b).toEqual({ echo: ["b"] });
    expect(maxExecuteInFlight).toBe(1);
    await w.close();
  });

  it("deletes the WebDriver session before reopening Marionette connections", async () => {
    const names: string[] = [];
    const { port } = await startMock((name) => {
      names.push(name);
    });
  ...
  it("reports disconnected and rejects new commands while close is in progress", async () => {
    let started!: () => void;
    ...
    const { port } = await startMock(async (name) => {
      if (name !== "WebDriver:DeleteSession") return;
      started();
    ...
    await expect(w.execute("return 1;")).rejects.toThrow("marionette not connected");
