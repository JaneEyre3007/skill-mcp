...
| `FRX_STATE_HOME` | MCP 自身状态根目录（convo / turnlog / Marionette 锁，非逆向产物），**默认 `<本 MCP 包根>/.frx-director-mcp`** |
| `FRX_DATA_DIR` | MCP 会话数据目录（convo / turnlog，非逆向产物），**默认 `<FRX_STATE_HOME>/data`** |
