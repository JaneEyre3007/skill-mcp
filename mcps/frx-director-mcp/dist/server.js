 ...
 */
export function registerTools(server, director) {
    server.registerTool("frx_status", {
        title: "Preflight: check setup before starting",
        description: "★开跑前先调这个自检:浏览器(marionette)连没连、当前 worker provider/model、worker key 配没配。" +
    ...
    server.registerTool("agent_start", {
        ...
        description: "开一个新的逆向会话:配置 worker 模型(默认沿用浏览器里已配的)+关掉工具确认门、(可选)导航到目标 URL、建工作目录、用便宜的 worker 模型以【AI 辅助模式】发出第一轮(task 作为第 0 条 user)。返回 tid,后续都用它。" +
    ...
    server.registerTool("agent_wait_for_stop", {
        title: "Wait for the worker's stage gate",
        description: "阻塞轮询直到 worker 这一轮 settled(阶段门到了)。这是'便宜模型磨完了、该读了'的信号。" +
    ...
    server.registerTool("agent_read", {
        title: "Read the worker's stage conclusion",
        description: "读 worker 的阶段结论(①发现 ②2-3 方向 ③推荐)给 director 评判:含完整 content 尾、步骤尾、工作目录 progress.md/ledger.md(worker 真正依赖的持久知识),以及 ★driftHint + runlogTail——" +
    ...
    server.registerTool("agent_state", {
        ...
        description: "廉价的存活/进度快照(比 agent_read 轻):running/settled/nSteps/checkpointSeq/错误/contentTail 短尾/driftHint。" +
    ...
    server.registerTool("agent_send", {
        ...
        description: "★director 的核心动作:把方向纠正作为下一条 user 消息发出去并起下一轮(=harness 的 advance)。" +
    ...
    server.registerTool("agent_set_mode", {
        title: "Switch assist/auto",
        description: "切会话模式:'assist'=每轮阶段门停给 director review(成本拆分默认);'auto'=worker 跑到完成不停。" +
    ...
    server.registerTool("agent_stop", {
        ...
        description: "砍掉 worker 当前这一轮(agentSession.stop)。用在:读到快照发现 worker 正冲向错误路线,想立刻截停并 agent_send 纠正、不想干等它 settle。进展已落盘(ledger/progress.md),停掉不丢持久状态。",
        inputSchema: { tid: z.string() },
    }, async (a) => {
        try {
    ...
    server.registerTool("agent_runlog", {
        title: "Engine run log (diagnostic)",
        description: "引擎级 run 日志(getRunLog):请求/响应时序、重试、idle 看门狗超时、finishReason、drift 检测。诊断用——让 director 区分'worker 真的判定路线走不通' vs '模型侧 drift / idle 超时 / 推理档模型(如 deepseek-v4-pro)吐了纯文字计划没调工具',下结论前先核它。",
    ...
    server.registerTool("agent_tools", {
        title: "List the browser-side tools",
        description: "列出浏览器侧可用的逆向工具清单(名称/说明/是否需确认/参数名)——含 16 个引擎级逆向工具(signer_trace/jsvmp_trace/closure_read/webapi_trace/whitebox_diff/wasm_probe 等,页面检测不到)。" +
    ...
    server.registerTool("agent_call_tool", {
        title: "Directly run ONE browser tool (bypass the worker)",
        description: "★你(director)亲自跑一个浏览器引擎工具,跳过 DeepSeek worker —— 强模型直驱引擎级工具,比委派 worker 磨更快更准。" +
