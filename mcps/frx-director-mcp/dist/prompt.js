 ...
 */
const FALLBACK_SYSTEM = "你是 firefox-reverse 浏览器内置的逆向 Agent。站点无关、通用。用你自己的工具走通常规逆向链路，" +
...
function loadFromJsx(jsxPath) {
    const { mtimeMs } = statSync(jsxPath);
    if (cache && cache.path === jsxPath && cache.mtime === mtimeMs)
        return cache;
    const txt = readFileSync(jsxPath, "utf8");
    const system = extract(txt, "SYSTEM");
    if (!system)
        throw new Error(`cannot extract SYSTEM from ${jsxPath}`);
    cache = {
        path: jsxPath,
        mtime: mtimeMs,
        system,
...
function workspaceBlock(workspace) {
    return (`\n\n【当前工作目录】${workspace}\n` +
...
export function buildSystemPrompt(jsxPath, workspace, mode) {
