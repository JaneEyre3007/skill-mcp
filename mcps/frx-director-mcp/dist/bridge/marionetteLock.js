...
export class MarionetteLock {
    ...
    isReclaimable(holder) {
        ...
        if (this.isOurs(holder))
            return true; // leftover from a prior incarnation of THIS process
    ...
    tryAcquire() {
        ...
        try {
            ...
            try {
                writeSync(fd, JSON.stringify({ ...this.self, ts: Date.now() }));
            }
            finally {
                closeSync(fd);
            }
    ...
    renew() {
        if (!this.held)
            return;
        if (Date.now() - this.lastWrite < RENEW_MIN_MS)
            return;
        const holder = this.currentHolder();
        if (!this.isOurs(holder)) {
        ...
        try {
            writeFileSync(this.path, JSON.stringify({ ...this.self, ts: Date.now() }));
            this.lastWrite = Date.now();
        }
        catch {
            /* ignore */
...
//# sourceMappingURL=marionetteLock.js.map