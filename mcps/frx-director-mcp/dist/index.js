...
function makeBridge() {
    if (config.bridge === "file")
        return new FileBridge(config.fileBridgeDir);
    return new MarionetteBridge(config.marionetteHost, config.marionettePort, undefined, config.stateHome);
}
async function main() {
    // stdio transport: all human-readable diagnostics MUST go to stderr (stdout is
    // the MCP JSON-RPC channel).
    const logErr = (...a) => console.error("[frx-director-mcp]", ...a);
    if (config.bridge === "marionette") {
        ...
        if (!res.reachable) {
            logErr(`Marionette ${config.marionetteHost}:${config.marionettePort} 不可达。` +
    ...
    logErr("ready — 11 tools registered, listening on stdio");
