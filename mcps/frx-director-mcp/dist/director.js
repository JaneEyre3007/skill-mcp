...
import { waitForStop } from "./settle.js";
...
export class Director {
    ...
    async confirmStarted(tid) {
        for (let i = 0; i < 8; i++) {
            await sleep(250);
            const st = await this.bridge.getState(tid);
            if (st && (st.running || st.settled || st.nSteps > 0))
                return true;
        }
        return false;
    }
    ...
    async status() {
        // Expose MCP's own configuration so skills can derive the Firefox root path
        // from frx_status directly, without reading any external config file.
        const mcpConfig = {
            firefoxBin: this.cfg.firefoxBin || null,
            marionetteHost: this.cfg.marionetteHost,
            marionettePort: this.cfg.marionettePort,
            profile: this.cfg.profile || null,
            workspaceRoot: this.cfg.workspaceRoot,
            autolaunch: this.cfg.autolaunch,
        };
        try {
            ...
            return {
                ...
                note: "连不上浏览器的 marionette(127.0.0.1:2828)。请用户用 " +
    ...
    async start(args) {
        ...
        if (!cfgRes.hasKey) {
            throw new Error(`当前 provider "${cfgRes.provider}" 没有配置 API key。` +
        ...
        const convo = [{ role: "user", content: args.task }];
    ...
    async read(args) {
        ...
        if (!st)
            return { tid: args.tid, settled: false, content: "", steps: [], note: "no state — turn never started?" };
    ...
    async state(args) {
        ...
        const st = await this.bridge.getState(args.tid);
        if (!st)
            return { tid: args.tid, running: false, settled: false, nSteps: 0, note: "no state" };
        return {
            tid: args.tid,
            running: st.running,
            settled: st.settled,
    ...
    async runlog(args) {
        ...
        const entries = args.tid
            ? all.filter((e) => e && typeof e === "object" && e.threadId === args.tid)
            : all;
    ...
    async tools() {
        ...
        return {
            ...
            note: "浏览器侧可用的逆向工具清单。默认走成本拆分(委派 worker:agent_start / agent_send);" +
