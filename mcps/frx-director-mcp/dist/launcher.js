...
export async function waitPort(host, port, timeoutSec) {
    const t0 = Date.now();
...
export async function ensureBrowser(opts) {
    if (await probe(opts.host, opts.port))
        return { launched: false, reachable: true };
    if (!opts.autolaunch) {
        return { launched: false, reachable: false };
    }
    if (!opts.firefoxBin)
        throw new Error("FRX_AUTOLAUNCH=1 but FRX_FIREFOX_BIN is unset");
