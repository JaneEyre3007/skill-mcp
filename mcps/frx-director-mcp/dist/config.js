...
const num = (v, d) => (v && !Number.isNaN(+v) ? +v : d);
...
// MCP 自身的内部状态（每会话 convo / turnlog、Marionette 锁，非逆向产物）。
// 默认放在 MCP 包根目录下，避免在用户主目录创建隐藏目录。
const FRX_STATE_HOME = env.FRX_STATE_HOME || join(PKG_ROOT, ".frx-director-mcp");
export const config = {
    ...
    stateHome: FRX_STATE_HOME,
    dataDir: env.FRX_DATA_DIR || join(FRX_STATE_HOME, "data"),
    /** Bridge transport. "marionette" is the proven default; "file" is a stub seam. */
    bridge: (env.FRX_BRIDGE || "marionette"),
    fileBridgeDir: env.FRX_FILE_BRIDGE_DIR || join(tmpdir(), "frx-bridge"),
};
