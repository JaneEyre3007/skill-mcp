...
export class MarionetteWire {
  ...
  async command(name: string, params: Record<string, unknown> = {}): Promise<unknown> {
    if (!this.sock || this.closed || this.closing) throw new Error("marionette not connected");
    const run = this.commandQueue.then(() => this.doCommand(name, params));
    this.commandQueue = run.catch(() => undefined);
    return run;
  }

  private async doCommand(name: string, params: Record<string, unknown> = {}): Promise<unknown> {
